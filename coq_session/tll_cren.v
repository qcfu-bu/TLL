...
Require Export AutosubstSsr ARS tll_ast sta_step.
...
Lemma term_cren_sta_step m n ξ : m ~> n -> term_cren m ξ ~> term_cren n ξ.
...
Lemma term_cren_sta_red m n ξ : m ~>* n -> term_cren m ξ ~>* term_cren n ξ.
Proof with eauto using sta_step.
  move=>rd. elim: rd ξ=>{n}...
  move=>y z rd ih st ξ.
  apply: starSE.
  apply: ih.
  by apply: term_cren_sta_step.
Qed.

Lemma term_cren_sta_conv m n ξ : m === n -> term_cren m ξ === term_cren n ξ.
Proof with eauto using sta_step.
  move=>rd. elim: rd ξ=>{n}...
  { move=>y z rd ih st ξ.
    apply: convSE.
