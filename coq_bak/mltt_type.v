...
| mltt0_axiom Γ l :
  mltt0_wf Γ ->
  mltt0_type Γ (Ty l) (Ty l.+1)
| mltt0_var Γ x A :
  mltt0_wf Γ ->
  mltt_has Γ x A ->
  mltt0_type Γ (Var x) A
| mltt0_pi Γ A B l1 l2 :
  mltt0_type Γ A (Ty l1) ->
  mltt0_type (A :: Γ) B (Ty l2) ->
  mltt0_type Γ (Pi A B) (Ty (maxn l1 l2))
| mltt0_lam Γ A B m l :
  mltt0_type Γ A (Ty l) ->
  mltt0_type (A :: Γ) m B ->
...
| mltt0_sig Γ A B l1 l2 :
  mltt0_type Γ A (Ty l1) ->
  mltt0_type (A :: Γ) B (Ty l2) ->
  mltt0_type Γ (Sig A B) (Ty (maxn l1 l2))
| mltt0_dpair Γ A B m n :
  mltt0_type Γ m A ->
  mltt0_type Γ n B.[m/] ->
  mltt0_type Γ (DPair m n) (Sig A B)
| mltt0_letin Γ A B C m n l :
  mltt0_type (Sig A B :: Γ) C (Ty l) ->
...
| mltt0_id Γ A m n l :
  ...
  mltt0_type Γ m A ->
  mltt0_type Γ n A ->
  mltt0_type Γ (Id A m n) (Ty l)
| mltt0_refl Γ A m :
  mltt0_type Γ m A ->
  mltt0_type Γ (Refl m) (Id A m m)
| mltt0_rw Γ A B H P m n l :
  mltt0_type (Id A.[ren (+1)] m.[ren (+1)] (Var 0) :: A :: Γ) B (Ty l) ->
...
| mltt_app Γ A B m n :
  ...
  Γ ⊢ n : A ->
  Γ ⊢ App m n : B.[n/]
| mltt_sig Γ A B l1 l2 :
  Γ ⊢ A : Ty l1 ->
  (A :: Γ) ⊢ B : Ty l2  ->
  Γ ⊢ Sig A B : Ty (maxn l1 l2)
| mltt_dpair Γ A B m n :
  Γ ⊢ m : A ->
  Γ ⊢ n : B.[m/] ->
  Γ ⊢ DPair m n : Sig A B
| mltt_letin Γ A B C m n l :
  (Sig A B :: Γ) ⊢ C : Ty l ->
  Γ ⊢ m : Sig A B ->
...
| mltt_refl Γ A m :
  Γ ⊢ m : A ->
  Γ ⊢ Refl m : Id A m m
| mltt_rw Γ A B H P m n l :
  (Id A.[ren (+1)] m.[ren (+1)] (Var 0) :: A :: Γ) ⊢ B : Ty l ->
  Γ ⊢ H : B.[Refl m,m/] ->
  Γ ⊢ P : Id A m n ->
  Γ ⊢ Rw B H P : B.[P,n/]
| mltt_conv Γ A B m l :
