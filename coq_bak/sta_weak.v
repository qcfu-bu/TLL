...
| sta_agree_ren_cons Γ Γ' ξ m s l :
  Γ ⊢ m : Sort s l ->
  sta_agree_ren ξ Γ Γ' ->
  sta_agree_ren (upren ξ) (m :: Γ) (m.[ren ξ] :: Γ')
| sta_agree_ren_wk Γ Γ' ξ m s l :
  Γ' ⊢ m : Sort s l ->
  sta_agree_ren ξ Γ Γ' ->
...
Lemma sta_agree_weak_wf_cons Γ Γ' A s ξ :
  ...
  (∀ Γ' ξ, sta_agree_ren ξ Γ Γ' → exists l, Γ' ⊢ A.[ren ξ] : Sort s l) ->
  sta_wf Γ'.
Proof with eauto using sta_wf.
...
Lemma sta_wf_ok Γ x A :
  sta_wf Γ -> sta_has Γ x A -> exists s l, Γ ⊢ A : Sort s l.
...
Lemma sta_eweaken Γ m m' A A' B s l :
  m' = m.[ren (+1)] ->
  A' = A.[ren (+1)] ->
  Γ ⊢ m : A ->
  Γ ⊢ B : Sort s l ->
