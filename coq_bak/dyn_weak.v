...
| dyn_agree_ren_ty Γ Γ' Δ Δ' ξ m s l :
  Γ ⊢ m : Sort s l ->
...
| dyn_agree_ren_wkU Γ Γ' Δ Δ' ξ m l :
  Γ' ⊢ m : Sort U l ->
  dyn_agree_ren ξ Γ Δ Γ' Δ' ->
  dyn_agree_ren (ξ >>> (+1)) Γ Δ (m :: Γ') (m :U Δ')
| dyn_agree_ren_wkN Γ Γ' Δ Δ' ξ m s l :
  Γ' ⊢ m : Sort s l ->
  dyn_agree_ren ξ Γ Δ Γ' Δ' ->
...
Lemma dyn_weakenN Γ Δ m A B s l :
  Γ ⊢ B : Sort s l ->
  Γ ; Δ ⊢ m : A ->
...
Lemma dyn_eweakenU Γ Δ m m' A A' B l :
  m' = m.[ren (+1)] ->
  A' = A.[ren (+1)] ->
  Γ ⊢ B : Sort U l ->
  Γ ; Δ ⊢ m : A ->
  (B :: Γ) ; B :U Δ ⊢ m' : A'.
Proof.
  move=>*; subst. apply: dyn_weakenU; eauto.
Qed.

Lemma dyn_eweakenN Γ Δ m m' A A' B s l :
  m' = m.[ren (+1)] ->
  A' = A.[ren (+1)] ->
  Γ ⊢ B : Sort s l ->
  Γ ; Δ ⊢ m : A ->
