...
let rec pp0_tm ctx0 fmt = function
  ...
  | Match (m, bnd, cls) ->
    ...
    pf fmt "match %a [ %a ⇒ %a ] with %a" (pp0_tm ctx0) m V.pp x (pp0_tm ctx) a
      (pp0_cls ctx0) cls
  (* equality *)
  ...
  | Rew (bnd, p, m) ->
    ...
    pf fmt "rew [ %a, %a ⇒ %a ] %a in %a" V.pp xs.(0) V.pp xs.(1) (pp0_tm ctx) a
      (pp0_tm ctx0) p (pp0_tm ctx0) m
  (* monadic *)
  ...
  | Act (R, rol, a, bnd) ->
    let x, b, ctx = unbind_in ctx0 bnd in
    pf fmt "%a(%a : %a) ⇒ %a" pp_role rol V.pp x (pp0_tm ctx0) a (pp0_tm ctx) b
  | Act (N, rol, a, bnd) ->
    let x, b, ctx = unbind_in ctx0 bnd in
    pf fmt "%a{%a : %a} ⇒ %a" pp_role rol V.pp x (pp0_tm ctx0) a (pp0_tm ctx) b
