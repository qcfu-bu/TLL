...
| pstep_letin m m' n n' :
  pstep m m' ->
  pstep n n' ->
  pstep (LetIn m n) (LetIn m' n')
| pstep_letret m m' n n' :
  pstep m m' ->
  pstep n n' ->
  pstep (LetIn (Return m) n) n'.[m'/]
...
Proof.
  ...
  { move=>m n σ.
    ...
    apply: sta_step_letret. autosubst. }
Qed.
...
Lemma sta_red_letin m m' n n' :
  m ~>* m' -> n ~>* n' -> LetIn m n ~>* LetIn m' n'.
Proof.
  move=>r1 r2.
  apply: (star_trans (LetIn m' n)).
  apply: (star_hom (LetIn^~ n)) r1=>x y. exact: sta_step_letinL.
  apply: (star_hom (LetIn m')) r2=>x y. exact: sta_step_letinR.
Qed.
...
Hint Resolve
  ...
  sta_red_io sta_red_return sta_red_letin
...
Lemma sta_conv_letin m m' n n' :
  m === m' -> n === n' -> LetIn m n === LetIn m' n'.
Proof.
  move=>r1 r2.
  apply: (conv_trans (LetIn m' n)).
  apply: (conv_hom (LetIn^~ n)) r1=>x y. exact: sta_step_letinL.
  apply: (conv_hom (LetIn m')) r2=>x y. exact: sta_step_letinR.
Qed.
...
Hint Resolve
  ...
  sta_conv_io sta_conv_return sta_conv_letin
...
Proof with eauto using pstep, pstep_refl.
  ...
  { move=>m m' n n' ps1 ih1 ps2 ih2 σ. asimpl.
    pose proof (pstep_letret (ih1 σ) (ih2 (up σ))) as H.
    by asimpl in H. }
Qed.
...
Proof with eauto 6 using pstep, psstep_up.
  ...
  { move=>m m' n n' ps1 ih1 ps2 ih2 σ τ pss. asimpl.
    pose proof (pstep_letret (ih1 _ _ pss)) (ih2 _ _ (psstep_up pss)) as H.
    by asimpl in H. }
Qed.
...
Proof with eauto 6 using
  ...
  { move=>m m' n n' pm ihm pn ihn m2 p. inv p.
    { have[m0 pm1 pm2]:=ihm _ H1.
      have[n0 pn1 pn2]:=ihn _ H3.
      exists (LetIn m0 n0)... }
    { have[m1 pm1 pm2]:=ihm _ (pstep_return H1).
...
Proof. elim=>//. move=>y z r1 e r2. subst. inv r2. Qed.

Lemma sta_red_it_inv n x : Num n ~>* x -> x = Num n.
Proof. elim=>//. move=>y z r1 e r2. subst. inv r2. Qed.

Lemma sta_red_nat_inv x : Nat ~>* x -> x = Nat.
Proof. elim=>//. move=>y z r1 e r2. subst. inv r2. Qed.

Lemma sta_red_num_inv n x : Num n ~>* x -> x = Num n.
Proof. elim=>//. move=>y z r1 e r2. subst. inv r2. Qed.

Lemma sta_red_rand_inv x : Rand ~>* x -> x = Rand.
Proof. elim=>//. move=>y z r1 e r2. subst. inv r2. Qed.

Lemma sta_red_io_inv A x :
  IO A ~>* x -> exists A', A ~>* A' /\ x = IO A'.
Proof.
  elim.
  exists A=>//.
  move=>y z r1[A'[rA e]]r2. subst.
  inv r2.
  exists A'0. eauto using star.
Qed.

Lemma sta_red_return_inv m x :
  Return m ~>* x -> exists m', m ~>* m' /\ x = Return m'.
Proof.
  elim.
  exists m=>//.
...
Ltac red_inv m H :=
  ...
  | It   => apply sta_red_it_inv in H
  | Unit => apply sta_red_unit_inv in H
  | Nat  => apply sta_red_nat_inv in H
  | Num  => apply sta_red_num_inv in H
  | Rand => apply sta_red_rand_inv in H
