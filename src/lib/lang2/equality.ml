...
let rec aeq_tm m1 m2 =
  if m1 == m2 then
    ...
    | Pi (rel1, s1, a1, bnd1), Pi (rel2, s2, a2, bnd2) ->
      rel1 = rel2 && eq_sort s1 s2 && aeq_tm a1 a2 && eq_binder aeq_tm bnd1 bnd2
    ...
    | Let (rel1, m1, bnd1), Let (rel2, m2, bnd2) ->
      rel1 = rel2 && aeq_tm m1 m2 && eq_binder aeq_tm bnd1 bnd2
    (* inductive *)
    ...
    | Match (ms1, a1, cls1), Match (ms2, a2, cls2) ->
      Array.for_all2
        (fun (m1, rel1) (m2, rel2) -> aeq_tm m1 m2 && rel1 = rel2)
        ms1 ms2
      && aeq_tm a1 a2
      && Array.for_all2 (fun cl1 cl2 -> eq_pbinder aeq_tm cl1 cl2) cls1 cls2
    | Absurd, Absurd -> true
    (* record *)
    | Record (s1, mp1), Record (s2, mp2) ->
