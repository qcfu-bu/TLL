...
let rec pp_tm fmt = function
  ...
  | Fix (x, bnd) ->
    ...
    pf fmt "@[fix (%a := %a) ⇒@;<1 2>%a@]" V.pp x V.pp r pp_tm m
  (* data *)
  | Sigma (R, U, a, bnd) ->
    let x, b = unbind bnd in
    if binder_occur bnd then
...
let rec pp_dcons fmt (DCons (c, ptl)) =
  pf fmt "| @[%a of@;<1 2>@[%a@]@]" C.pp c pp_ptl ptl
...
let rec pp_dcl fmt = function
  | DTm (R, x, a, m) ->
    pf fmt "@[@[program %a :@;<1 2>%a@;<1 0>=@]@;<1 2>%a@]" V.pp x pp_tm a pp_tm
      m
  | DTm (N, x, a, m) ->
    pf fmt "@[@[logical %a :@;<1 2>%a@;<1 0>=@]@;<1 2>%a@]" V.pp x pp_tm a pp_tm
      m
  | DData (d, ptm, dconss) ->
    pf fmt "@[<v 0>@[inductive %a@;<1 2>%a@;<1 0>=@]@;<1 0>@[%a@]@]" D.pp d
      pp_ptm ptm pp_dconss dconss
