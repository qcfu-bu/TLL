...
and match_cls ?(expand = true) (env : Env.t) guard cls ms =
  ...
  and psubst (p0s, bnd) ms =
    let rec match_p0 p0 m =
      ...
      | P0Constr (c1, p0s) -> (
        match whnf ~expand env m with
        | Constr (c2, _, _, ms) when Constr.equal c1 c2 -> match_p0s p0s ms
        | _ -> failwith "equality1e.match_p0")
      | _ -> failwith "equality12.match_p0"
...
let rec aeq_tm m1 m2 =
  if m1 == m2 then
    ...
    | Fun (guard1, a1, bnd1), Fun (guard2, a2, bnd2) ->
      guard1 = guard2 && aeq_tm a1 a2
    ...
    | Match (guard1, ms1, a1, cls1), Match (guard2, ms2, a2, cls2) ->
      guard1 = guard2 && List.equal aeq_tm ms1 ms2 && aeq_tm a1 a2
    ...
    | Act (relv1, role1, a1, bnd1), Act (relv2, role2, a2, bnd2) ->
      relv1 = relv2 && role1 = role2 && aeq_tm a1 a2
      && eq_binder aeq_tm bnd1 bnd2
  ...
  let rec equal m1 m2 =
    if aeq_tm m1 m2 then
      ...
      | Constr (c1, ss1, ms1, ns1), Constr (c2, ss2, ms2, ns2) ->
        ...
        && List.equal equal ms1 ms2 && List.equal equal ns1 ns2
      | Match (guard1, ms1, a1, cls1), Match (guard2, ms2, a2, cls2) ->
        guard1 = guard2 && List.equal equal ms1 ms2 && equal a1 a2
      ...
      | End, End -> true
      | Act (relv1, role1, a1, bnd1), Act (relv2, role2, a2, bnd2) ->
        relv1 = relv2 && role1 = role2 && equal a1 a2
        && eq_binder equal bnd1 bnd2
