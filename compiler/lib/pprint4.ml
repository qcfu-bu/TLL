...
let rec pp_cmd fmt = function
  ...
  | MkConstr { lhs; fip; ctag; args } -> (
    match fip with
    | Some m ->
      pf fmt "@[%a := reconstr(%a, %a, [%a]);@]" Name.pp lhs pp_expr m Constr.pp
        ctag pp_exprs args
    | None ->
      pf fmt "@[%a := mkconstr(%a, [%a]);@]" Name.pp lhs Constr.pp ctag pp_exprs
        args)
  | Match0 { cond; cases } ->
    pf fmt "@[match(%a){@;<1 2>@[%a@]@;<1 0>}@]" pp_expr cond pp_cases0 cases
  | Match1 { cond; sort; cases } ->
    pf fmt "@[match[%a](%a){@;<1 2>@[%a@]@;<1 0>}@]" pp_sort sort pp_expr cond
      pp_cases1 cases
  | Absurd -> pf fmt "absurd;"
  (* lazy *)
  | Thunk { lhs; cmds; ret } ->
    pf fmt
      "@[%a :=@;<1 2>@[thunk {@;<1 2>@[<v 0>%a@;<1 0>return %a;@]@;<1 0>}@];@]"
  ...
  | Add (lhs, m, n) ->
    pf fmt "@[%a := add(%a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Sub (lhs, m, n) ->
    pf fmt "@[%a := sub(%a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Mul (lhs, m, n) ->
    pf fmt "@[%a := mul(%a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Div (lhs, m, n) ->
    pf fmt "@[%a := div(%a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Mod (lhs, m, n) ->
    pf fmt "@[%a := mod(%a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Lte (lhs, m, n) ->
    pf fmt "@[%a := lte(%a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Gte (lhs, m, n) ->
    pf fmt "@[%a := gte(%a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Lt (lhs, m, n) ->
    pf fmt "@[%a := lt(%a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Gt (lhs, m, n) ->
    pf fmt "@[%a := gt(%a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Eq (lhs, m, n) ->
    pf fmt "@[%a := eq(%a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Chr (lhs, m) -> pf fmt "@[%a := chr(%a);@]" Name.pp lhs pp_expr m
  | Ord (lhs, m) -> pf fmt "@[%a := ord(%a);@]" Name.pp lhs pp_expr m
  | Str (lhs, s) ->
    pf fmt "@[%a := str(\"%s\");@]" Name.pp lhs (String.escaped s)
  | Push (lhs, m, n) ->
    pf fmt "@[%a := push(%a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Cat (lhs, m, n) ->
    pf fmt "@[%a := cat(%a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Size (lhs, m) -> pf fmt "@[%a := size(%a);@]" Name.pp lhs pp_expr m
  | Indx (lhs, m, n) ->
    pf fmt "@[%a := indx(%a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  (* primitive effects *)
  ...
  | Send (lhs, m, n) ->
    pf fmt "@[%a := send(%a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Recv (lhs, sort, m) ->
    pf fmt "@[%a := recv[%a](%a);@]" Name.pp lhs pp_sort sort pp_expr m
  | Close (lhs, role, m) ->
    pf fmt "@[%a := close[%b](%a);@]" Name.pp lhs role pp_expr m
  (* magic *)
  | Magic -> pf fmt "magic;"
...
and pp_case0 fmt = function
  | Case (c, rhs) ->
    pf fmt "@[<v 0>%a => {@;<1 2>%a@;<1 0>}@]" Constr.pp c pp_cmds rhs
...
and pp_case1 fmt case =
  ...
  in
  pf fmt "@[<v 0>%a(%a) => {@;<1 2>%a@;<1 0>}@]" Constr.pp case.ctag pp_args
    case.args pp_cmds case.rhs
...
let pp_dcl fmt = function
  ...
  | DefVal { lhs; cmds; ret } ->
    pf fmt "@[<v 0>%a := {@;<1 2>@[<v 0>%a@;<1 0>return %a;@]@;<1 0>};@]"
      Name.pp lhs pp_cmds cmds pp_expr ret
