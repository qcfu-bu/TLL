...
let rec fv_cmds bound = function
  ...
  | Fun { lhs; fn; args; cmds; ret } :: rest ->
    let bound =
      List.fold_left (fun acc (_, x) -> Fv.add x acc) (Fv.add fn bound) args
    in
  ...
  | App { lhs; fn; args } :: rest ->
    ...
    let fv2 =
      List.fold_left
        (fun acc (_, e) -> Fv.union acc (fv_expr bound e))
        Fv.empty args
    in
    let fv3, bound = fv_cmds (Fv.add lhs bound) rest in
    (Fv.(union (union fv1 fv2) fv3), bound)
  ...
  | MkConstr { lhs; fip; args } :: rest ->
    ...
    in
    let fv2 =
      List.fold_left (fun acc e -> Fv.union acc (fv_expr bound e)) Fv.empty args
    in
    let fv3, bound = fv_cmds (Fv.add lhs bound) rest in
    (Fv.(union (union fv1 fv2) fv3), bound)
...
let trans_expr = function
  | Var x -> Syntax5.Var x
  | Ctag c -> Syntax5.(Box (Ctag c))
  | Int i -> Syntax5.(Box (Int i))
  | Char c -> Syntax5.(Box (Char c))
  | NULL -> Syntax5.NULL
...
let rec trans_cmds (ctx : Ctx.t) lift = function
  ...
  | Fun { lhs; fn; args; cmds; ret } :: rest ->
    ...
    let cmds1 =
      List.mapi (fun i x -> Syntax5.(Env (x, i))) ((fn :: fvs) @ xs)
    in
    let cmds2, lift = trans_cmds ctx lift cmds in
    let cmds3 =
      List.mapi (fun i x -> Syntax5.(SetClo (lhs, Var x, i + 1))) fvs
    in
    let ret = trans_expr ret in
    let rest, lift = trans_cmds ctx lift rest in
    Syntax5.
      ( MkClo1
          { lhs; fn = fname; fvc = List.length fvs; argc = List.length args }
        :: cmds3
        @ rest
      , DefFun1 { fn = fname; cmds = cmds1 @ cmds2; ret } :: lift )
  | App { lhs; fn; args } :: rest -> (
    let rest, lift = trans_cmds ctx lift rest in
          ...
          (fun (lhs, rhs, acc) (s, arg) ->
            ...
            Syntax5.(
              ...
              | L ->
                ( Name.mk "x"
                , lhs
                , Free (Var rhs) :: AppC { lhs; fn = rhs; arg } :: acc )))
          (Name.mk "x", fn, [])
          args
      in
  ...
  | MkConstr { lhs; fip; ctag; args } :: rest -> (
    let cmds =
      List.mapi (fun i e -> Syntax5.SetBox (lhs, trans_expr e, i)) args
    in
    let rest, lift = trans_cmds ctx lift rest in
    match fip with
    | Some e ->
      Syntax5.((ReBox { lhs; fip = trans_expr e; ctag } :: cmds) @ rest, lift)
    | None ->
      Syntax5.
        ((MkBox { lhs; ctag; argc = List.length args } :: cmds) @ rest, lift))
  | Match0 { cond; cases } :: rest ->
    let cond = trans_expr cond in
    let cases, lift = trans_cases0 ctx lift cond cases in
    let rest, lift = trans_cmds ctx lift rest in
    Syntax5.(Switch { cond = Unbox cond; cases } :: rest, lift)
    ...
    | _ ->
      let cases =
        List.map
          ...
          cases
      in
      let x = Name.mk "tag" in
      Syntax5.(GetTag (x, cond) :: Switch { cond = Var x; cases } :: rest, lift)
    )
  | Absurd :: rest ->
    let rest, lift = trans_cmds ctx lift rest in
  ...
  (* lazy *)
  | Thunk { lhs; cmds; ret } :: rest ->
    let fname = Name.mk "thunk_" in
    let fv1, bound = fv_cmds Fv.empty cmds in
    let fv2 = fv_expr bound ret in
    let fvs = Fv.(dump (union fv1 fv2)) in
    let cmds1 = List.mapi (fun i x -> Syntax5.(Env (x, i))) fvs in
    let cmds2, lift = trans_cmds ctx lift cmds in
    let cmds3 = List.mapi (fun i x -> Syntax5.(SetThunk (lhs, Var x, i))) fvs in
    let ret = trans_expr ret in
    let rest, lift = trans_cmds ctx lift rest in
    Syntax5.
      ( (Thunk { lhs; fn = fname; fvc = List.length fvs } :: cmds3) @ rest
      , DefFun1 { fn = fname; cmds = cmds1 @ cmds2; ret } :: lift )
  | Force (lhs, e) :: rest ->
    let e = trans_expr e in
    let rest, lift = trans_cmds ctx lift rest in
  ...
  | Close (lhs, role, e) :: rest ->
    ...
    let rest, lift = trans_cmds ctx lift rest in
    if role then
      Syntax5.(Close0 (lhs, e) :: rest, lift)
    else
      Syntax5.(Close1 (lhs, e) :: rest, lift)
  (* magic *)
  | Magic :: rest ->
    let rest, lift = trans_cmds ctx lift rest in
...
and trans_cases1 ctx lift cond cases =
  let lift, cases =
    List.fold_left_map
      (fun lift { ctag; args; rhs } ->
        let cmds1 =
          List.mapi
            (fun i (x, occur) -> Syntax5.(GetBox (x, cond, i), occur))
            args
        in
        let cmds1 =
          List.filter_map
...
let trans_dcls dcls : Syntax5.prog =
  let rec aux ctx lift = function
    ...
    | DefFun { fn; args; cmds; ret } :: rest ->
      ...
      Syntax5.
        ( DefFun0 { fn = fname0; args = xs; cmds = cmds0; ret = ret0 }
          :: DefFun1 { fn = fname1; cmds = cmds1; ret = Var ret1 }
          :: lift
        , MkClo0 { lhs = fn; fn = fname1; fvc = 0; argc } :: rest
        , r )
