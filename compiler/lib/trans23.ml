...
let rec trans_tm ctx = function
  ...
  (* monad *)
  | Return m -> Syntax3.(_Thunk (trans_tm ctx m))
  | MLet _ as m ->
  ...
  | Print m ->
    let m = trans_tm ctx m in
    Syntax3.(_Thunk (_Print m))
  | Prerr m ->
    let m = trans_tm ctx m in
    Syntax3.(_Thunk (_Prerr m))
  | ReadLn m ->
    let m = trans_tm ctx m in
    Syntax3.(_Thunk (_ReadLn m))
  | Fork m ->
    let m = trans_tm ctx m in
    Syntax3.(_Thunk (_Fork m))
  | Send (R, _, m) ->
    let x = Syntax3.(Var.mk "x") in
    let m = Syntax3.(_Thunk (_Send (trans_tm ctx m) (_Var x))) in
    Syntax3.(_Lam U (bind_var x m))
  | Send (N, _, m) ->
    let x = Syntax3.(Var.mk "x") in
    let m = Syntax3.(_Thunk (trans_tm ctx m)) in
    Syntax3.(_Lam U (bind_var x m))
  | Recv (R, s, m) -> Syntax3.(_Thunk (_Recv (trans_sort s) (trans_tm ctx m)))
  | Recv (N, _, m) -> Syntax3.(_Thunk (trans_tm ctx m))
  | Close (role, m) -> Syntax3.(_Thunk (_Close role (trans_tm ctx m)))
  (* erasure *)
  | NULL -> Syntax3._NULL
  (* magic *)
