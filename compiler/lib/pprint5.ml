...
let rec pp_expr fmt = function
  | Box m -> pf fmt "box(%a)" pp_expr m
  | Unbox m -> pf fmt "unbox(%a)" pp_expr m
  | Var n -> Name.pp fmt n
...
let rec pp_cmd fmt = function
  ...
  | AppC { lhs; fn; arg } ->
    pf fmt "appc(%a, %a, %a);" Name.pp lhs Name.pp fn pp_expr arg
  | Free m -> pf fmt "@[free(%a);@]" pp_expr m
  (* inductive *)
  | MkBox { lhs; ctag; argc } ->
    pf fmt "mkbox(%a, %a, %d);" Name.pp lhs Constr.pp ctag argc
  | ReBox { lhs; fip; ctag } ->
    pf fmt "rebox(%a, %a, %a);" Name.pp lhs pp_expr fip Constr.pp ctag
  | SetBox (lhs, e, i) -> pf fmt "set_box(%a, %a, %d);" Name.pp lhs pp_expr e i
  | GetBox (lhs, e, i) -> pf fmt "get_box(%a, %a, %d);" Name.pp lhs pp_expr e i
  | GetTag (lhs, e) -> pf fmt "get_tag(%a, %a);" Name.pp lhs pp_expr e
  | Switch { cond; cases } ->
  ...
  (* lazy *)
  | Thunk { lhs; fn; fvc } ->
    pf fmt "thunk(%a, %a, %d);" Name.pp lhs Name.pp fn fvc
  | SetThunk (lhs, e, i) ->
    pf fmt "set_thunk(%a, %a, %d);" Name.pp lhs pp_expr e i
  | Force (lhs, m) -> pf fmt "@[force(%a, %a);@]" Name.pp lhs pp_expr m
  (* primitive operators *)
  | Neg (lhs, m) -> pf fmt "@[neg(%a, %a);@]" Name.pp lhs pp_expr m
  | Add (lhs, m, n) ->
    pf fmt "@[add(%a, %a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Sub (lhs, m, n) ->
    pf fmt "@[sub(%a, %a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Mul (lhs, m, n) ->
    pf fmt "@[mul(%a, %a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Div (lhs, m, n) ->
    pf fmt "@[div(%a, %a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Mod (lhs, m, n) ->
    pf fmt "@[mod(%a, %a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Lte (lhs, m, n) ->
    pf fmt "@[lte(%a, %a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Gte (lhs, m, n) ->
    pf fmt "@[gte(%a, %a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Lt (lhs, m, n) ->
    pf fmt "@[lt(%a, %a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Gt (lhs, m, n) ->
    pf fmt "@[gt(%a, %a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Eq (lhs, m, n) ->
    pf fmt "@[eq(%a, %a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  ...
  | Push (lhs, m, n) ->
    pf fmt "@[push(%a, %a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Cat (lhs, m, n) ->
    pf fmt "@[cat(%a, %a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  | Size (lhs, m) -> pf fmt "@[size(%a, %a);@]" Name.pp lhs pp_expr m
  | Indx (lhs, m, n) ->
    pf fmt "@[indx(%a, %a, %a);@]" Name.pp lhs pp_expr m pp_expr n
  (* primitive effects *)
  ...
  | Send (lhs, m, n) ->
    pf fmt "@[send(%a, %a, %a);@]" Name.pp lhs pp_expr m pp_expr n
...
and pp_case fmt = function
  | Case (ctag, rhs) ->
    pf fmt "@[<v 0>%a => {@;<1 2>%a@;<1 0>}@]" Constr.pp ctag pp_cmds rhs
