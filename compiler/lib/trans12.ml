...
module Logical = struct
  ...
  and infer_tm ctx env m =
    ...
    | Prerr m ->
      ...
      IO (Ind (base_ind, [ U ], [], []))
    | ReadLn m ->
      check_tm ctx env m (Ind (base_ind, [ U ], [], []));
      IO String_t
    | Fork m -> (
      let t = infer_tm ctx env m in
      match whnf env t with
...
module Program = struct
  ...
  let rec infer_tm ctx env m =
    ...
    | Lte (m, n) ->
      ...
      let n_elab, usg2 = check_tm ctx env n Int_t in
      Syntax2.
        (Ind (bool_ind, [], [], []), _Lte m_elab n_elab, Usage.merge usg1 usg2)
    | Gte (m, n) ->
      let m_elab, usg1 = check_tm ctx env m Int_t in
      let n_elab, usg2 = check_tm ctx env n Int_t in
      Syntax2.
        (Ind (bool_ind, [], [], []), _Gte m_elab n_elab, Usage.merge usg1 usg2)
    | Lt (m, n) ->
      let m_elab, usg1 = check_tm ctx env m Int_t in
      let n_elab, usg2 = check_tm ctx env n Int_t in
      Syntax2.
        (Ind (bool_ind, [], [], []), _Lt m_elab n_elab, Usage.merge usg1 usg2)
    | Gt (m, n) ->
      let m_elab, usg1 = check_tm ctx env m Int_t in
      let n_elab, usg2 = check_tm ctx env n Int_t in
      Syntax2.
        (Ind (bool_ind, [], [], []), _Gt m_elab n_elab, Usage.merge usg1 usg2)
    | Eq (m, n) ->
      let m_elab, usg1 = check_tm ctx env m Int_t in
      let n_elab, usg2 = check_tm ctx env n Int_t in
      Syntax2.
        (Ind (bool_ind, [], [], []), _Eq m_elab n_elab, Usage.merge usg1 usg2)
    | Chr m ->
      let m_elab, usg = check_tm ctx env m Int_t in
    ...
    | Send m -> (
      ...
      | Ch (role1, Act (relv, role2, a, bnd)) when role1 = role2 ->
        ...
        Syntax2.
          ( Pi (relv, L, a, bnd)
          , _Send (trans_relv relv) (trans_sort s) m_elab
          , usg )
...
let warn_constr x e =
  match e with
  | Failure s ->
    epr "@[<v 0>warning - pruned constructor %a@;<1 2>![@[%s@]]@]@." Constr.pp x
      s
  | _ -> raise e
      ...
      List.fold_right
        (fun ss (dcl_elab, ctx_acc) ->
          ...
          try
            ...
            let dcs_elab, ctx_acc, cs0 =
              check_dconstrs ss ctx env relv d0 dcs ctx_acc
            in
            let ctx_acc = Ctx.add_ind d1 (arity, cs0) ctx_acc in
            Syntax2.
              ( Inductive
                  { name = d1; relv = Program.trans_relv relv; body = dcs_elab }
                :: dcl_elab
              , ctx_acc )
          with
          | e ->
