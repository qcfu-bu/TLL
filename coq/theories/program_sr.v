...
Proof with eauto using logical_step.
  ...
  { move=>Γ A B m n s tym ihm tyn ihn e n0 st. inv st.
    ...
    { apply: star1... }
    { apply: star1... } }
  { move=>Γ A B m n t l tyS ihS tym ihm tyn ihn e n0 st. inv st.
    apply: logical_red_pair0... }
  { move=>Γ A B m n t l tyS ihS tym ihm tyn ihn e n0 st. inv st.
    { apply: logical_red_pair1... }
    { apply: logical_red_pair1... } }
  { move=>Γ A B C m n s t l tyC ihC tym ihm tyn ihn e n0 st. inv st.
  ...
  { move=>Γ A B H P m n s l tyB ihB tyH ihH tyP ihP e n0 st. inv st.
    ...
    apply: star1... }
  { move=>Γ A B m s l eq tym ihm tyB ihB e n st. subst.
    apply: ihm... }
Qed.
...
Lemma program_has_type Γ Δ x s A :
  program_has Δ x s A -> program_wf Γ Δ -> exists l, Γ ⊢ A : Sort s l.
Proof with eauto.
  move=>hs. elim: hs Γ=>{Δ x s A}.
  { move=>Δ A s k Γ wf. inv wf.
    exists l. apply: logical_eweaken... by asimpl. }
  { move=>Δ A B x s hs ih Γ wf. inv wf.
    have[lA tyA]:=ih _ H2. 
    exists lA. apply: logical_eweaken... by asimpl. }
...
Lemma program_val_key Γ Δ m A s l :
  Γ ; Δ ⊢ m : A -> Γ ⊢ A : Sort s l -> program_val m -> Δ ▷ s.
Proof with eauto using key_impure.
  destruct s...
  move=>ty. elim: ty l=>{Γ Δ m A}.
  { move=>Γ Δ x s A shs dhs wf l0 tyA vl.
    have[l tyAs]:=program_has_type wf shs.
    have e:=logical_unicity tyA tyAs. subst.
    apply: program_has_key... }
  { move=>Γ Δ A B m s k tym ih l0 tyP vl.
    have[t[l1[l2[tyB/sort_inj[e1 e2]]]]]:=logical_pi0_inv tyP. by subst. }
  { move=>Γ Δ A B m s t k tym ih l0 tyP vl.
    have[r[l1[l2[tyB/sort_inj[e1 e2]]]]]:=logical_pi1_inv tyP. by subst. }
  { move=>Γ Δ A B m n s tym ih tyn l0 tyB vl. inv vl. }
  { move=>Γ Δ1 Δ2 Δ A B m n s mrg tym ihm tyn ihn l0 tyB vl. inv vl. }
  { move=>Γ Δ A B m n t l tyS tym ihm tyn l0 ty vl.
    have[s[r[l1[l2[ord[tyA[tyB/sort_inj[e1 e2]]]]]]]]:=logical_sig0_inv ty. subst.
    inv ord. inv vl... }
  { move=>Γ Δ1 Δ2 Δ A B m n t l mrg tyS tym ihm tyn ihn l0 ty vl.
    have[s[r[l1[l2[ord1[ord2[tyA[tyB/sort_inj[e1 e2]]]]]]]]]:=logical_sig1_inv ty. subst.
...
Proof with eauto using program_type, program_step, program_wf, merge.
  ...
  { move=>Γ Δ A B m n s tym ihm tyn e1 e2 n0 st. inv st.
    ...
    { have tyn':=logical_rd tyn (program_logical_step tyn H2).
      have[t[lP tyP]]:=program_valid tym.
      have[r[l1[l2[tyB _]]]]:=logical_pi0_inv tyP.
      apply: program_conv.
      ...
      asimpl... }
    { have[x[lP tyP]]:=program_valid tym.
  ...
  { move=>Γ Δ1 Δ2 Δ A B m n s mrg tym ihm tyn ihn e1 e2 n0 st.
    ...
    { have tyn':=ihn erefl erefl _ H2.
      have[x[lP tyP]]:=program_valid tym.
      have[r[l1[l2[tyB _]]]]:=logical_pi1_inv tyP.
      apply: program_conv.
      apply: logical_conv_beta.
      apply: conv_sym.
      apply: star_conv.
      apply: program_logical_step...
      apply: program_app1...
      have:=logical_subst tyB (program_logical_type tyn).
      asimpl... }
    { exfalso.
    ...
    { have[x[lP tyP]]:=program_valid tym.
      ...
      apply: (program_val_key tyn)... } }
  { move=>Γ Δ A B m n t l tyS tym ihm tyn e1 e2 n0 st. inv st.
    have[s[r[l1[l2[ord[tyA[tyB _]]]]]]]:=logical_sig0_inv tyS.
    apply: program_pair0...
    apply: logical_conv.
    apply: logical_conv_beta.
    apply: star_conv.
    apply: (program_logical_step (program_logical_type tym))...
    apply: tyn.
    apply: logical_esubst...
    by autosubst. }
  { move=>Γ Δ1 Δ2 Δ A B m n t l mrg tyS tym ihm tyn ihn e1 e2 n0 st.
    subst. inv mrg. inv st.
    { have[s[r[l1[l2[ord1[ord2[tyA[tyB _]]]]]]]]:=logical_sig1_inv tyS.
      ...
      by autosubst. }
    { apply: program_pair1... } }
  { move=>Γ Δ1 Δ2 Δ A B C m n s r t l mrg tyC tym ihm tyn ihn e1 e2 n0 st.
    subst. inv mrg. inv st.
    { apply: program_conv.
      apply: logical_conv_beta.
      apply: conv_sym.
    ...
    { inv H3.
      ...
      apply: program_val_key tym1 H7 H0.
      apply: merge_sym...
      apply: program_agree_subst_refl...
      by autosubst. }
    { exfalso. apply: logical_pair1_sig0_false... } }
  { move=>Γ Δ1 Δ2 Δ A B C m n s r1 r2 t l mrg tyC tym ihm tyn ihn e1 e2 n0 st.
    subst. inv mrg. inv st.
    { apply: program_conv.
      apply: logical_conv_beta.
      apply: conv_sym.
    ...
    { inv H3.
      ...
      have k1:=program_val_key tym1 H8 H1.
      have k2:=program_val_key tym2 (logical_subst H6 (program_logical_type tym1)) H4.
