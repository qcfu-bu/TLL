...
Require Export AutosubstSsr ARS
  logical_ctx logical_step logical_type
  program_ctx program_step.
...
| program_ff Γ Δ :
  ...
  Δ ▷ U ->
  Γ ; Δ ⊢ FF : Bool
| program_ifte Γ Δ1 Δ2 Δ A m n1 n2 s l :
  Δ1 ∘ Δ2 => Δ ->
  (Bool :: Γ) ⊢ A : Sort s l ->
  Γ ; Δ1 ⊢ m : Bool ->
  Γ ; Δ2 ⊢ n1 : A.[TT/] ->
...
| program_rw Γ Δ A B H P m n s l :
  (Id A.[ren (+1)] m.[ren (+1)] (Var 0) :: A :: Γ) ⊢ B : Sort s l ->
  Γ ; Δ ⊢ H : B.[Refl m,m/] ->
  Γ ⊢ P : Id A m n ->
  Γ ; Δ ⊢ Rw B H P : B.[P,n/]
| program_conv Γ Δ A B m s l :
  A ≃ B ->
  Γ ; Δ ⊢ m : A ->
  Γ ⊢ B : Sort s l ->
  Γ ; Δ ⊢ m : B
...
| program_wf_ty Γ Δ A s l :
  program_wf Γ Δ ->
  Γ ⊢ A : Sort s l ->
  program_wf (A :: Γ) (A .{s} Δ)
| program_wf_n Γ Δ A s l :
  program_wf Γ Δ ->
  Γ ⊢ A : Sort s l ->
  program_wf (A :: Γ) (_: Δ).
...
Proof with eauto using program_wf.
  ...
  { move=>Γ Δ1 A s l wf1 ih tyA Δ Δ2 mrg wf2. inv mrg.
...
Proof with eauto using program_wf.
  ...
  { move=>Γ Δ1 Δ2 Δ A B m n t mrg tyS tym ihm tyn ihn.
    apply: program_wf_merge... }
  { move=>Γ Δ1 Δ2 Δ A B C m n s r t l mrg tyC tym ihm tyn ihn.
    inv ihn. inv H2.
    apply: program_wf_merge... }
...
Proof with eauto using program_wf.
  ...
  { move=>Γ Δ A s l wf ih tyA Δ1 Δ2 mrg. inv mrg.
