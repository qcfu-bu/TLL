...
| erasure_agree_subst_nil σ :
  nil ; nil ⊢ σ ~ σ ⊣ nil ; nil
| erasure_agree_subst_ty Γ1 Δ1 σ1 σ2 Γ2 Δ2 A s l :
  Γ1 ; Δ1 ⊢ σ1 ~ σ2 ⊣ Γ2 ; Δ2 ->
  Γ2 ⊢ A : Sort s l ->
  (A.[σ1] :: Γ1) ; (A.[σ1] .{s} Δ1) ⊢ up σ1 ~ up σ2 ⊣ (A :: Γ2) ; (A .{s} Δ2)
| erasure_agree_subst_n Γ1 Δ1 σ1 σ2 Γ2 Δ2 A s l :
  Γ1 ; Δ1 ⊢ σ1 ~ σ2 ⊣ Γ2 ; Δ2 ->
  Γ2 ⊢ A : Sort s l ->
...
| erasure_agree_subst_conv0 Γ1 Δ1 σ1 σ2 Γ2 Δ2 A B s l :
  A ≃ B ->
  Γ1 ⊢ B.[ren (+1)].[σ1] : Sort s l ->
  Γ2 ⊢ B : Sort s l ->
  Γ1 ; Δ1 ⊢ σ1 ~ σ2 ⊣ (A :: Γ2) ; (_: Δ2) ->
  Γ1 ; Δ1 ⊢ σ1 ~ σ2 ⊣ (B :: Γ2) ; (_: Δ2)
| erasure_agree_subst_conv1 Γ1 Δ1 σ1 σ2 Γ2 Δ2 A B s l :
  A ≃ B ->
  Γ1 ⊢ B.[ren (+1)].[σ1] : Sort s l ->
  Γ2 ⊢ B : Sort s l ->
...
Proof with eauto using key.
  ...
  { move=>Γ1 Δ1 σ1 σ2 Γ2 Δ2 A s l agr ih tyA r k. inv k... }
  { move=>Γ1 Δ1 σ1 σ2 Γ2 Δ2 A s l agr ih tyA r k. inv k... }
  ...
  { move=>Γ1 Δ1 σ1 σ2 Γ2 Δ2 A B s l eq tyB1 tyB2 agr ih r k. inv k... }
Qed.
...
Proof with eauto using erasure_agree_subst, erasure_agree_subst_key.
  ...
  { move=>Γ1 Δ1 σ1 σ2 Γ2 Δ2 A B s l eq tyB1 tyB2 agr ih x t C wf hs. inv hs.
    { apply: erasure_conv.
...
Proof with eauto 6 using merge, erasure_agree_subst, erasure_agree_subst_key.
  ...
  { move=>Γ1 Δ1 σ1 σ2 Γ2 Δ2 A s l agr ih tyA Δa Δb mrg. inv mrg.
  ...
  { move=>Γ1 Γ2 σ1 σ2 Δ1 Δ2 Δa Δb n n' A s k mrg agr ih tyn Δa' Δb' mrg'.
    ...
    { have[Δa'[Δb'[mrg'[agra agrb]]]]:=ih _ _ H2.
      ...
      repeat split...
      exact: merge_sym. } }
  { move=>Γ1 Δ1 σ1 σ2 Γ2 Δ2 A B s l eq tyB1 tyB2 agr ih Δa Δb mrg. inv mrg.
...
Proof with eauto using erasure_agree_subst, erasure_agree_subst_key, erasure_type.
  ...
  { move=>Γ Δ1 Δ2 Δ A B m m' n n' s mrg tym ihm tyn ihn
    ...
    apply: erasure_app1... }
  { move=>Γ Δ A B m m' n t l tyS tym ihm tyn Γ1 Δ1 σ1 σ2 agr. asimpl.
  ...
  { move=>Γ Δ1 Δ2 Δ A B m m' n n' t l mrg tyS tym ihm tyn ihn Γ1 Δ0 σ1 σ2 agr. asimpl.
    have[Δa[Δb[mrg0[agra agrb]]]]:=erasure_agree_subst_merge agr mrg.
    have{}ihS:=logical_substitution tyS (program_logical_agree_subst (erasure_program_agree_subst agr)).
    have{}ihm:=ihm _ _ _ _ agra.
    have{}ihn:=ihn _ _ _ _ agrb.
    apply: erasure_pair1...
    asimpl. asimpl in ihn... }
  { move=>Γ Δ1 Δ2 Δ A B C m m' n n' s r t l mrg tyC tym ihm tyn ihn Γ1 Δ0 σ1 σ2 agr. asimpl.
    have[Δa[Δb[mrg0[agra agrb]]]]:=erasure_agree_subst_merge agr mrg.
    ...
    asimpl. asimpl in ihn... }
  { move=>Γ Δ1 Δ2 Δ A B C m m' n n' s r1 r2 t l mrg tyC tym ihm tyn ihn Γ1 Δ0 σ1 σ2 agr. asimpl.
    have[Δa[Δb[mrg0[agra agrb]]]]:=erasure_agree_subst_merge agr mrg.
    replace C.[m.[σ1] .: σ1] with C.[up σ1].[m.[σ1]/] by autosubst.
    have wf:=logical_type_wf tyC. inv wf.
    have wf:=program_type_wf (erasure_program_type tyn). inv wf. inv H4.
  ...
  { move=>Γ Δ wf k Γ1 Δ1 σ1 σ2 agr. asimpl.
    have wf':=program_substitution_wf wf (erasure_program_agree_subst agr).
    apply: erasure_tt... }
  ...
  { move=>Γ Δ1 Δ2 Δ A m m' n1 n1' n2 n2' s l mrg tyA tym ihm
    ...
    have[Δa[Δb[mrgx[agra agrb]]]]:=erasure_agree_subst_merge agr mrg.
    have wf:=logical_type_wf tyA. inv wf.
    have{}tyA:=logical_substitution tyA (logical_agree_subst_ty (erasure_logical_agree_subst agr) H2).
    have{}ihm:=ihm _ _ _ _ agra.
  ...
  { move=>Γ Δ A B H H' P m n s l tyB erH ihH tyP Γ1 Δ1 σ1 σ2 agr. asimpl.
    ...
    asimpl in ihP... }
  { move=>Γ Δ A B m m' s l eq tym ihm tyB Γ1 Δ1 σ1 σ2 agr.
    apply: erasure_conv.
    apply: logical_conv_subst...
    apply: ihm...
...
Lemma erasure_ctx_conv0 Γ Δ m m' A B C s l :
  B ≃ A ->
  Γ ⊢ B : Sort s l -> (A :: Γ) ; _: Δ ⊢ m ~ m' : C -> (B :: Γ) ; _: Δ ⊢ m ~ m' : C.
...
Lemma erasure_ctx_conv1 Γ Δ m m' A B C s l :
  B ≃ A ->
