...
Lemma program_lam0_invX Γ Δ A1 A2 B C m s1 s2 t l :
  Γ ; Δ ⊢ Lam0 A1 m s1 : C ->
  C ≃ Pi0 A2 B s2 ->
  (A2 :: Γ) ⊢ B : Sort t l ->
  (A2 :: Γ) ; _: Δ ⊢ m : B.
...
Lemma program_lam1_invX Γ Δ A1 A2 B C m s1 s2 t l :
  Γ ; Δ ⊢ Lam1 A1 m s1 : C ->
  C ≃ Pi1 A2 B s2 ->
  (A2 :: Γ) ⊢ B : Sort t l ->
...
Lemma program_lam0_inv Γ Δ m A1 A2 B s1 s2 :
  Γ ; Δ ⊢ Lam0 A2 m s2 : Pi0 A1 B s1 -> (A1 :: Γ) ; _: Δ ⊢ m : B.
Proof with eauto.
  move=>ty.
  have[t[l/logical_pi0_inv[r[l1[l2[tyB _]]]]]]:=program_valid ty.
  apply: program_lam0_invX...
Qed.

Lemma program_lam1_inv Γ Δ m A1 A2 B s1 s2 :
  Γ ; Δ ⊢ Lam1 A2 m s2 : Pi1 A1 B s1 -> exists r, (A1 :: Γ) ; A1 .{r} Δ ⊢ m : B.
Proof with eauto.
  move=>ty.
  have[t[l/logical_pi1_inv[r[l1[l2[tyB _]]]]]]:=program_valid ty.
  apply: program_lam1_invX...
Qed.

Lemma program_pair0_invX Γ Δ A B m n s r t l C :
  Γ ; Δ ⊢ Pair0 m n s : C ->
  C ≃ Sig0 A B r ->
...
Proof with eauto.
  ...
  { move=>Γ Δ A B m n t l ty1 tym ihm tyn A0 B0 m0 n0 s r t0 l0[e1 e2 e3]
    ...
    have tym0:Γ ; Δ ⊢ m : A0 by apply: program_conv; eauto.
    repeat split...
    apply: logical_conv.
...
Lemma program_pair1_invX Γ Δ A B m n s r t l C :
  Γ ; Δ ⊢ Pair1 m n s : C ->
  C ≃ Sig1 A B r ->
...
Proof with eauto.
  ...
  { move=>Γ Δ1 Δ2 Δ A B m n t l mrg ty1 tym _ tyn _ A0 B0 m0 n0 s r t0 l0
    ...
    have tym0:Γ ; Δ1 ⊢ m : A0 by apply: program_conv; eauto.
    repeat split...
    apply: program_conv.
    apply: logical_conv_subst.
    all: eauto.
...
Lemma program_pair0_inv Γ Δ A B m n s r :
  Γ ; Δ ⊢ Pair0 m n s : Sig0 A B r ->
  s = r /\ Γ ; Δ ⊢ m : A /\ Γ ⊢ n : B.[m/].
Proof with eauto.
  move=>ty.
  have[t[l tyS]]:=program_valid ty.
  apply: program_pair0_invX...
Qed.

Lemma program_pair1_inv Γ Δ A B m n s r :
