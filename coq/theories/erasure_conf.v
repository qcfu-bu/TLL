...
| program_pstep_tt : program_pstep TT TT
| program_pstep_ff : program_pstep FF FF
| program_pstep_ifte A m m' n1 n2 :
  program_pstep m m' ->
  program_pstep (Ifte A m n1 n2) (Ifte A m' n1 n2)
| program_pstep_ifteT A n1 n2 :
  program_pstep (Ifte A TT n1 n2) n1
| program_pstep_ifteF A n1 n2 :
  program_pstep (Ifte A FF n1 n2) n2
...
Lemma program_red_ifte A m m' n1 n2 :
  m ~>>* m' -> Ifte A m n1 n2 ~>>* Ifte A m' n1 n2.
Proof.
  move=>r.
  apply: (star_hom (((Ifte A)^~ n1)^~ n2)) r=>x y. exact: program_step_ifteM.
Qed.
...
Proof with eauto.
  ...
  { move=>Γ Δ1 Δ2 Δ A B m m' n n' s mrg erm ihm ern ihn nx ps. inv ps.
    ...
    { exfalso.
      ...
      have{}H0:=program_logical_type (erasure_program_type erm).
  ...
  { move=>Γ Δ1 Δ2 Δ A m m' n1 n1' n2 n2' s l
    ...
    { apply: program_red_ifte... }
    { apply: starES. apply: program_step_ifteT. apply: starR. }
    { apply: starES. apply: program_step_ifteF. apply: starR. } }
...
Proof with eauto using program_pstep, erasure_pstep_reflexive.
  ...
  { move=>Γ Δ1 Δ2 Δ A B m m' n n' s mrg erm ihm ern ihn m1 m2 ps1 ps2.
    ...
    { inv H1.
      ...
      have wf:=program_type_wf (erasure_program_type erm1). inv wf.
      have vl:=erasure_program_val ern H4.
      have k:=program_val_key (erasure_program_type ern) H6 vl.
  ...
  { move=>Γ Δ1 Δ2 Δ A B C m m' n n' s r t l
    ...
    { inv ps2.
      ...
      { inv H3. inv H4.
        ...
        have wf:=program_type_wf (erasure_program_type ern). inv wf. inv H3.
        have[vl e]:=program_val_pstep H0 H5. subst.
        have vl':=erasure_program_val erm1 vl.
        have k:=program_val_key (erasure_program_type erm1) H8 vl'.
        have agr:
          Γ ; Δ ⊢ (m3 .: m2 .: ids) ~
    ...
    { inv H3.
      ...
      have k:=program_val_key (erasure_program_type erm1) H7 vl.
      have agr:
        Γ ; Δ ⊢ (m4 .: m1 .: ids) ~
  ...
  { move=>Γ Δ1 Δ2 Δ A B C m m' n n' s r1 r2 t l
    ...
    { inv ps2.
      ...
      { exfalso.
        ...
        apply: logical_pair0_sig1_false... }
      { inv H3. inv H4.
        have wf:=program_type_wf (erasure_program_type ern). inv wf. inv H4.
        ...
        have//=tyB:=logical_subst H8 (program_logical_type (erasure_program_type erm1)).
        have k1:=program_val_key (erasure_program_type erm1) H10 vl3.
        have k2:=program_val_key (erasure_program_type erm2) tyB vl4.
    ...
    { inv H3.
      ...
      have//=tyB:=logical_subst H6 (program_logical_type (erasure_program_type erm1)).
      have k1:=program_val_key (erasure_program_type erm1) H8 vl3.
      have k2:=program_val_key (erasure_program_type erm2) tyB vl4.
      have[Δx[mrg3 mrg4]]:=merge_splitL mrg mrg1.
      have agr:
        Γ ; Δ ⊢ (m5 .: m4 .: ids) ~
  ...
  { move=>Γ Δ1 Δ2 Δ A m m' n1 n1' n2 n2' s l
    ...
    { inv H4. exists m2. constructor. apply: erasure_pstep_reflexive... }
    { inv H4. exists m2. constructor. apply: erasure_pstep_reflexive... }
    { inv H4. exists m1. apply: erasure_pstep_reflexive... constructor. }
    { inv H4. exists m1. apply: erasure_pstep_reflexive... constructor. } }
