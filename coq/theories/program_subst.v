...
| program_agree_subst_nil σ :
  nil ; nil ⊢ σ ⊣ nil ; nil
| program_agree_subst_ty Γ1 Δ1 σ Γ2 Δ2 A s l :
  Γ1 ; Δ1 ⊢ σ ⊣ Γ2 ; Δ2 ->
  Γ2 ⊢ A : Sort s l ->
  (A.[σ] :: Γ1) ; (A.[σ] .{s} Δ1) ⊢ up σ ⊣ (A :: Γ2) ; (A .{s} Δ2)
| program_agree_subst_n Γ1 Δ1 σ Γ2 Δ2 A s l :
  Γ1 ; Δ1 ⊢ σ ⊣ Γ2 ; Δ2 ->
  Γ2 ⊢ A : Sort s l ->
...
| program_agree_subst_conv0 Γ1 Δ1 σ Γ2 Δ2 A B s l :
  A ≃ B ->
  Γ1 ⊢ B.[ren (+1)].[σ] : Sort s l ->
  Γ2 ⊢ B : Sort s l ->
  Γ1 ; Δ1 ⊢ σ ⊣ (A :: Γ2) ; (_: Δ2) ->
  Γ1 ; Δ1 ⊢ σ ⊣ (B :: Γ2) ; (_: Δ2)
| program_agree_subst_conv1 Γ1 Δ1 σ Γ2 Δ2 A B s l :
  A ≃ B ->
  Γ1 ⊢ B.[ren (+1)].[σ] : Sort s l ->
  Γ2 ⊢ B : Sort s l ->
  Γ1 ; Δ1 ⊢ σ ⊣ (A :: Γ2) ; (A .{s} Δ2) ->
...
Proof with eauto using program_agree_subst, program_agree_subst_key, program_type.
  ...
  { move=>Γ Δ1 Δ2 Δ A B C m n s r1 r2 t l mrg tyC tym ihm tyn ihn Γ1 Δ0 σ agr. asimpl.
    ...
    asimpl. asimpl in ihn... }
  { move=>Γ Δ wf ih k Γ1 Δ1 σ agr. asimpl. apply: program_tt... }
  { move=>Γ Δ wf ih k Γ1 Δ1 σ agr. asimpl. apply: program_ff... }
  { move=>Γ Δ1 Δ2 Δ A m n1 n2 s l mrg tyA tym ihm
    ...
    have[Δa[Δb[mrgx[agra agrb]]]]:=program_agree_subst_merge agr mrg.
    have wf:=logical_type_wf tyA. inv wf.
    have{}tyA:=logical_substitution tyA (logical_agree_subst_ty (program_logical_agree_subst agr) H2).
    have{}ihm:=ihm _ _ _ agra.
...
Lemma program_ctx_conv0 Γ Δ m A B C s l :
  B ≃ A ->
...
Lemma program_ctx_conv1 Γ Δ m A B C s l :
  B ≃ A ->
