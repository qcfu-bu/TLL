...
Lemma heap_srX H1 H2 H H' x y z z' A :
  H1 ; x ~ y ~ z : A -> wr_heap H ->
  H1 ∘ H2 => H -> H ; z ~>> H' ; z' ->
  exists H1' H2' x' y',
    H1' ; x' ~ y' ~ z' : A /\ wr_heap H' /\
    pad H2 H2' /\ H1' ∘ H2' => H' /\ x ~>>* x' /\ y ~>>* y'.
Proof with eauto 7 using erasure_type, resolve, program_step, key, merge, hmrg_setmU, hmrg_setmL, pad_refl.
  ...
  { move=>Γ Δ A B m m' n s erm ihm tyn H1 H2 H H' z z' e1 e2 rs wr mrg st; subst.
    ...
    { have[H1'[mrg' rs]]:=resolve_free H9 H6 mrg. inv rs.
      ...
      repeat split...
      { have[ern0 _]:=erasure_lam0_inv erm.
        apply: erasure_esubst0... }
      { have[ern _]:=erasure_lam0_inv erm.
  ...
  { move=>Γ Δ1 Δ2 Δ A B m m' n n' s mrg1 erm ihm ern ihn H1 H2 H H' z z' e1 e2 rs wr mrg2 st; subst.
    ...
    { have[H4[mrg3 mrg4]]:=hmrg_splitL mrg2 H7.
      ...
      repeat split...
      { have[t[lP tyP]]:=program_valid (erasure_program_type erm).
        have[r[l1[l2[tyB/sort_inj[e1 e2]]]]]:=logical_pi1_inv tyP. subst.
        have tyn:=program_logical_type (erasure_program_type ern).
    ...
    { have[H4[mrg3 mrg4]]:=hmrg_splitR mrg2 H7.
      ...
      { apply: erasure_subst1.
        ...
        apply: ern0.
        apply: ern. }
      { have wf:=program_type_wf (erasure_program_type ern0). inv wf.
  ...
  { move=>Γ Δ A B m m' n t l tyS erm ihm tyn H1 H2 H H' z z' e1 e2 rs wr mrg st; subst.
    ...
    { have[H1'[H2'[x'[y'[wrx[wr'[pd[mrg'[rd1 rd2]]]]]]]]]:=ihm _ _ _ _ _ _ erefl erefl H6 wr mrg H9.
      ...
      { have[s[r[l1[l2[ord[tyA[tyB _]]]]]]]:=logical_sig0_inv tyS.
        have tyx:=program_logical_type (program_rd (erasure_program_type erm) rd1).
        have//=tyBx:=logical_subst tyB tyx.
        econstructor...
        apply: logical_conv.
        apply: logical_conv_beta.
        apply: star_conv.
        apply: (program_logical_red (program_logical_type (erasure_program_type erm)))...
        apply: tyn.
        apply: tyBx. }
  ...
  { move=>Γ Δ1 Δ2 Δ A B m m' n n' t l mrg1 tyS erm ihm ern ihn H1 H2 H H' z z' e1 e2 rs wr mrg2 st; subst.
    have[s[r[l1[l2[ord1[ord2[tyA[tyB _]]]]]]]]:=logical_sig1_inv tyS. inv mrg1.
    have[wr1 wr2]:=wr_merge_inv mrg2 wr.
    inv rs; inv st.
  ...
  { move=>Γ Δ1 Δ2 Δ A B C m m' n n' s r t l mrg1 tyC
    ...
    { have[H4[mrg3 mrg4]]:=hmrg_splitR mrg2 H7.
      ...
      have vl:=wr_resolve_heap wr5 H15.
      have wf:=program_type_wf (erasure_program_type ern). inv wf.  inv H9.
  ...
  { move=>Γ Δ1 Δ2 Δ A B C m m' n n' s r1 r2 t l mrg1 tyC
    ...
    { have[H4[mrg3 mrg4]]:=hmrg_splitR mrg2 H7.
      ...
      have[Hx[mrg5 mrg6]]:=hmrg_splitL (hmrg_sym mrg') mrg3.
      have[m1[m2 e]]:=erasure_pair1_form erm. subst.
      have[Δ1[Δ2[mrg[e[erm1 erm2]]]]]:=erasure_pair1_inv erm. inv mrg; subst.
      have wr':=free_wr H13 wr.
      have[wr1 wr2]:=wr_merge_inv mrg2 wr.
      have[wr3 wr4]:=wr_merge_inv H7 wr1.
      have[wr5 wr6]:=wr_merge_inv mrg' wr'.
      have[wr7 wr8]:=wr_merge_inv H16 wr5.
      have vm:=wr_resolve_heap wr7 H18.
      have vn:=wr_resolve_heap wr8 H19.
      have wf:=program_type_wf (erasure_program_type ern). inv wf.  inv H14.
      have k1:=resolution (Ptr_well_resolved erm1 H18) H21 vm wr7.
      have//=tyBm1:=logical_subst H17 (program_logical_type (erasure_program_type erm1)).
  ...
  { move=>Γ Δ1 Δ2 Δ A m m' n1 n1' n2 n2' s l mrg1 tyAQ erm ihm
    ...
    { have[H4[mrg3 mrg4]]:=hmrg_splitR mrg2 H8.
      have[H1'[mrg' rs]]:=resolve_free H15 H11 (hmrg_sym mrg4). inv rs.
      have[Hx[mrg5 mrg6]]:=hmrg_splitL (hmrg_sym mrg') mrg3.
      have e:=erasure_ff_form erm. subst.
      have e:=hmrg_pureR mrg5 H6. subst.
      have wr':=free_wr H15 wr.
