...
| logical_agree_subst_nil σ :
  nil ⊢ σ ⊣ nil
| logical_agree_subst_ty Γ1 σ Γ2 A s l :
  Γ1 ⊢ σ ⊣ Γ2 ->
  Γ2 ⊢ A : Sort s l ->
  (A.[σ] :: Γ1) ⊢ up σ ⊣ (A :: Γ2)
| logical_agree_subst_wk Γ1 σ Γ2 n A :
  ...
  Γ1 ⊢ n : A.[σ] ->
  Γ1 ⊢ n .: σ ⊣ (A :: Γ2)
| logical_agree_subst_conv Γ1 σ Γ2 A B s l :
  A ≃ B ->
...
Proof with eauto using logical_agree_subst, logical_type.
  ...
  { move=>Γ A B C m n s t l tyC ihC tym ihm tyn ihn Γ1 σ agr. asimpl.
    ...
    asimpl in ihn...
    by asimpl. }
  { move=>Γ A m n1 n2 s l tyA ihA tym ihm tyn1 ihn1 tyn2 ihn2 Γ1 σ agr. asimpl.
    have wf:=logical_type_wf tyA. inv wf.
    have/={}ihA:=ihA _ _ (logical_agree_subst_ty agr H2).
    have/={}ihm:=ihm _ _ agr.
...
Lemma logical_ctx_conv Γ m A B C s l :
  B ≃ A ->
