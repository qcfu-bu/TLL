...
| logical_agree_ren_cons Γ Γ' ξ m s l :
  Γ ⊢ m : Sort s l ->
  logical_agree_ren ξ Γ Γ' ->
  logical_agree_ren (upren ξ) (m :: Γ) (m.[ren ξ] :: Γ')
| logical_agree_ren_wk Γ Γ' ξ m s l :
  Γ' ⊢ m : Sort s l ->
  logical_agree_ren ξ Γ Γ' ->
...
Lemma logical_agree_weak_wf_cons Γ Γ' A s ξ :
  ...
  (∀ Γ' ξ, logical_agree_ren ξ Γ Γ' → exists l, Γ' ⊢ A.[ren ξ] : Sort s l) ->
  logical_wf Γ'.
Proof with eauto using logical_wf.
  move e:(A :: Γ)=>Γ0 agr. elim: agr Γ A s e=>//{Γ0 Γ' ξ}...
  move=>Γ Γ' ξ m l s tym agr _ Γ0 A s0 [e1 e2] wf h1 h2; subst.
...
Proof with eauto using logical_type, logical_wf, logical_agree_ren.
  ...
  { move=>Γ A B s r t l1 l2 tyA ihA tyB ihB Γ' ξ agr. asimpl.
  ...
  { move=>Γ A B m n s tym ihm tyn ihn Γ' ξ agr. asimpl.
    ...
    apply: logical_app1...
    asimpl in ihm... }
  { move=>Γ A B s r t l1 l2 ord tyA ihA tyB ihB Γ' ξ agr. asimpl.
    have{}ihA:=ihA _ _ agr.
    have{}ihB:=ihB _ _ (logical_agree_ren_cons tyA agr).
    apply: logical_sig0... }
  { move=>Γ A B s r t l1 l2 ord1 ord2 tyA ihA tyB ihB Γ' ξ agr. asimpl.
    have{}ihA:=ihA _ _ agr.
    have{}ihB:=ihB _ _ (logical_agree_ren_cons tyA agr).
    apply: logical_sig1.
    exact: ord1. exact: ord2. all: eauto. }
  { move=>Γ A B m n t l tyS ihS tym ihm tyn ihn Γ' ξ agr. asimpl.
  ...
  { move=>Γ A B m n t l tyS ihS tym ihm tyn ihn Γ' ξ agr. asimpl.
    ...
    apply: logical_pair1...
    by autosubst. }
  { move=>Γ A B C m n s t l tyC ihC tym ihm tyn ihn Γ' ξ agr. asimpl.
  ...
  { move=>Γ A m n1 n2 s l tyA ihA tym ihm tyn1 ihn1 tyn2 ihn2 Γ' ξ agr. asimpl.
    ...
    have/={}ihA:=ihA _ _ (logical_agree_ren_cons H2 agr).
    have/={}ihm:=ihm _ _ agr.
  ...
  { move=>Γ A B H P m n s l tyB ihB tyH ihH tyP ihP Γ' ξ agr. asimpl.
    ...
    by autosubst. }
  { move=>Γ A B m s l eq tym ihm tyB ihB Γ' ξ agr.
    apply: logical_conv.
  ...
  { move=>Γ A s l wf ih tyA ihA Γ' ξ agr.
    apply: logical_agree_weak_wf_cons... }
Qed.

Lemma logical_wf_ok Γ x A :
  logical_wf Γ -> logical_has Γ x A -> exists s l, Γ ⊢ A : Sort s l.
Proof with eauto using logical_agree_ren, logical_agree_ren_refl.
  move=>wf. elim: wf x A=>{Γ}.
  { move=>x A hs. inv hs. }
  { move=>Γ A s l wf ih tyA x B hs. inv hs.
    { exists s. exists l.
      replace (Sort s l) with (Sort s l).[ren (+1)] by autosubst.
      apply: logical_rename... }
    { have[t[l0 tyA0]]:=ih _ _ H3.
      exists t. exists l0.
      replace (Sort t l0) with (Sort t l0).[ren (+1)] by autosubst.
      apply: logical_rename... } }
Qed.

Lemma logical_weaken Γ m A B s l :
  Γ ⊢ m : A ->
  Γ ⊢ B : Sort s l ->
  (B :: Γ) ⊢ m.[ren (+1)] : A.[ren (+1)].
Proof with eauto using logical_agree_ren, logical_agree_ren_refl.
...
Lemma logical_eweaken Γ m m' A A' B s l :
  m' = m.[ren (+1)] ->
  A' = A.[ren (+1)] ->
  Γ ⊢ m : A ->
  Γ ⊢ B : Sort s l ->
