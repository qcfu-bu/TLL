...
| program_agree_ren_ty Γ Γ' Δ Δ' ξ m s l :
  Γ ⊢ m : Sort s l ->
  program_agree_ren ξ Γ Δ Γ' Δ' ->
  program_agree_ren (upren ξ)
    (m :: Γ) (m .{s} Δ) (m.[ren ξ] :: Γ') (m.[ren ξ] .{s} Δ')
| program_agree_ren_n Γ Γ' Δ Δ' ξ m s l :
  Γ ⊢ m : Sort s l ->
  program_agree_ren ξ Γ Δ Γ' Δ' ->
  program_agree_ren (upren ξ)
    (m :: Γ) (_: Δ) (m.[ren ξ] :: Γ') (_: Δ')
| program_agree_ren_wkU Γ Γ' Δ Δ' ξ m l :
  Γ' ⊢ m : Sort U l ->
  program_agree_ren ξ Γ Δ Γ' Δ' ->
  program_agree_ren (ξ >>> (+1)) Γ Δ (m :: Γ') (m :U Δ')
| program_agree_ren_wkN Γ Γ' Δ Δ' ξ m s l :
  Γ' ⊢ m : Sort s l ->
  program_agree_ren ξ Γ Δ Γ' Δ' ->
...
Proof with eauto using key.
  ...
  { move=>Γ Γ' Δ Δ' ξ m s l tym agr ih t k. inv k... }
  { move=>Γ Γ' Δ Δ' ξ _ _ _ _ agr ih s k. inv k... }
  { move=>Γ Γ' Δ Δ' ξ m _ _ agr ih [] /ih... }
Qed.
...
Proof with eauto using program_agree_ren_key.
  ...
  { move=>Γ Γ' Δ Δ' ξ m s l tym agr ih x t A hs. inv hs; asimpl.
  ...
  { move=>Γ Γ' Δ Δ' ξ m s l tym agr ih x t A hs. inv hs; asimpl.
    replace A0.[ren (ξ >>> (+1))] with A0.[ren ξ].[ren (+1)] by autosubst.
     constructor... }
  { move=>Γ Γ' Δ Δ' ξ m l tym agr ih x s A hs.
    replace A.[ren (ξ >>> (+1))] with A.[ren ξ].[ren (+1)] by autosubst.
    constructor... }
  { move=>Γ Γ' Δ Δ' ξ m s l tym agr ih x t A hs.
    replace A.[ren (ξ >>> (+1))] with A.[ren ξ].[ren (+1)] by autosubst.
    constructor... }
Qed.
...
Proof with eauto 6 using merge, program_agree_ren.
  ...
  { move=>Γ Γ' Δ Δ' ξ m s l tym agr ih Δ1 Δ2 mrg. inv mrg.
  ...
  { move=>Γ Γ' Δ Δ' ξ m l tym agr ih Δ1 Δ2 mrg.
    have[Δ1'[Δ2'[mrg'[agr1 agr2]]]]:=ih _ _ mrg.
    exists (m :U Δ1'). exists (m :U Δ2')... }
  { move=>Γ Γ' Δ Δ' ξ m s l tym agr ih Δ1 Δ2 mrg.
    have[Δ1'[Δ2'[mrg'[agr1 agr2]]]]:=ih _ _ mrg.
    exists (_: Δ1'). exists (_: Δ2')... }
Qed.
...
Proof with eauto using program_wf.
  ...
  apply: program_wf_ty...
  replace (Sort s l) with (Sort s l).[ren ξ] by eauto.
...
Proof with eauto using program_wf.
  ...
  move=>Γ Γ' Δ Δ' ξ m s l tym agr _ A Γ0 Δ0 [e1 e2][e4]wf h; subst.
  apply: program_wf_n...
...
Proof with eauto using
  ...
  { move=>Γ Δ1 Δ2 Δ A B m n s mrg tym ihm tyn ihn Γ' Δ' ξ agr. asimpl.
    ...
    apply: program_app1...
    asimpl in ihm... }
  { move=>Γ Δ A B m n t l tyS tym ihm tyn Γ' Δ' ξ agr. asimpl.
    have{}ihm:=ihm _ _ _ agr.
    have{}ihn:=logical_rename tyn (program_logical_agree_ren agr).
  ...
  { move=>Γ Δ1 Δ2 Δ A B m n t l mrg tyS tym ihm tyn ihn Γ' Δ' ξ agr. asimpl.
    have[Δ1'[Δ2'[mrg'[agr1 agr2]]]]:=program_agree_ren_merge agr mrg.
    have{}ihm:=ihm _ _ _ agr1.
    have{}ihn:=ihn _ _ _ agr2.
    have{}ihS:=logical_rename tyS (program_logical_agree_ren agr).
    ...
    asimpl in ihS...
    asimpl. asimpl in ihn... }
  { move=>Γ Δ1 Δ2 Δ A B C m n s r t l mrg tyC tym ihm tyn ihn Γ' Δ' ξ agr. asimpl.
    ...
    by autosubst. }
  { move=>Γ Δ1 Δ2 Δ A B C m n s r1 r2 t l mrg tyC tym ihm tyn ihn Γ' Δ' ξ agr. asimpl.
    have wf:=logical_type_wf tyC. inv wf.
    have wf:=program_type_wf tyn. inv wf. inv H4.
    have[Δ1'[Δ2'[mrg'[agr1 agr2]]]]:=program_agree_ren_merge agr mrg.
    have{}ihC:=logical_rename tyC (logical_agree_ren_cons H2 (program_logical_agree_ren agr)).
    ...
    by autosubst. }
  { move=>Γ Δ wf ih k Γ' Δ' ξ agr. asimpl. apply:program_tt... }
  { move=>Γ Δ wf ih k Γ' Δ' ξ agr. asimpl. apply:program_ff... }
  ...
  { move=>Γ Δ A B H P m n s l tyB tyH ihH tyP Γ' Δ' ξ agr. asimpl.
    ...
    by autosubst. }
  { move=>Γ Δ A B m s l eq tym ihm tyB Γ' Δ' ξ agr.
    apply: program_conv.
  ...
  { move=>Γ Δ A s l wf ih tyA Γ' Δ' ξ agr.
...
Lemma program_weakenU Γ Δ m A B l :
  Γ ⊢ B : Sort U l ->
  Γ ; Δ ⊢ m : A ->
...
Lemma program_weakenN Γ Δ m A B s l :
  Γ ⊢ B : Sort s l ->
  Γ ; Δ ⊢ m : A ->
...
Lemma program_eweakenU Γ Δ m m' A A' B l :
  m' = m.[ren (+1)] ->
  A' = A.[ren (+1)] ->
  Γ ⊢ B : Sort U l ->
  Γ ; Δ ⊢ m : A ->
  (B :: Γ) ; B :U Δ ⊢ m' : A'.
Proof.
  move=>*; subst. apply: program_weakenU; eauto.
Qed.

Lemma program_eweakenN Γ Δ m m' A A' B s l :
  m' = m.[ren (+1)] ->
  A' = A.[ren (+1)] ->
  Γ ⊢ B : Sort s l ->
  Γ ; Δ ⊢ m : A ->
  (B :: Γ) ; _: Δ ⊢ m' : A'.
Proof.
  move=>*; subst. apply: program_weakenN; eauto.
Qed.
