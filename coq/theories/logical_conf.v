...
| pstep_heap l : pstep (Ptr l) (Ptr l).
...
Lemma logical_red_ifte A A' m m' n1 n1' n2 n2' :
  A ~>* A' -> m ~>* m' -> n1 ~>* n1' -> n2 ~>* n2' -> Ifte A m n1 n2 ~>* Ifte A' m' n1' n2'.
Proof.
  move=>rA rm rn1 rn2.
  apply: (star_trans (Ifte A' m n1 n2)).
  apply: (star_hom (((Ifte^~ m)^~ n1)^~ n2)) rA=>x y. exact: logical_step_ifteA.
  apply: (star_trans (Ifte A' m' n1 n2)).
  apply: (star_hom (((Ifte A')^~ n1)^~ n2)) rm=>x y. exact: logical_step_ifteM.
  apply: (star_trans (Ifte A' m' n1' n2)).
  apply: (star_hom ((Ifte A' m')^~ n2)) rn1=>x y. exact: logical_step_ifteN1.
  apply: (star_hom (Ifte A' m' n1')) rn2=>x y. exact: logical_step_ifteN2.
Qed.
...
#[global] Hint Resolve
  ...
  logical_red_letin logical_red_ifte
...
Lemma logical_conv_ifte A A' m m' n1 n1' n2 n2' :
  A ≃ A' -> m ≃ m' -> n1 ≃ n1' -> n2 ≃ n2' -> Ifte A m n1 n2 ≃ Ifte A' m' n1' n2'.
Proof.
  move=>rA rm rn1 rn2.
  apply: (conv_trans (Ifte A' m n1 n2)).
  apply: (conv_hom (((Ifte^~ m)^~ n1)^~ n2)) rA=>x y. exact: logical_step_ifteA.
  apply: (conv_trans (Ifte A' m' n1 n2)).
  apply: (conv_hom (((Ifte A')^~ n1)^~ n2)) rm=>x y. exact: logical_step_ifteM.
  apply: (conv_trans (Ifte A' m' n1' n2)).
  apply: (conv_hom ((Ifte A' m')^~ n2)) rn1=>x y. exact: logical_step_ifteN1.
  apply: (conv_hom (Ifte A' m' n1')) rn2=>x y. exact: logical_step_ifteN2.
Qed.
...
#[export] Hint Resolve
  ...
  logical_conv_letin logical_conv_ifte
...
Lemma logical_red_sort_inv s l A :
...
Lemma logical_red_bool_inv x : Bool ~>* x -> x = Bool.
Proof.
  elim=>//.
...
Lemma logical_red_tt_inv x : TT ~>* x -> x = TT.
Proof.
  elim=>//.
  move=>y z rd e st. subst.
  inv st.
...
Lemma logical_red_ff_inv x : FF ~>* x -> x = FF.
Proof.
  elim=>//.
  move=>y z rd e st. subst.
  inv st.
...
Ltac red_inv m H :=
  ...
  | Bool  => apply logical_red_bool_inv in H
  | TT    => apply logical_red_tt_inv in H
  | FF    => apply logical_red_ff_inv in H
