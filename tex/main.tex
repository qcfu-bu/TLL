...
\begin{theorem}[Static Subject Reduction]
  For any static typing $\Gamma \vdash m : A$ and static reduction $m \leadsto n$, then $\Gamma \vdash n : A$ is derivable.
\end{theorem}

\begin{theorem}[Static Normalization]
  For $m$ with static typing $\Gamma \vdash m : A$, it is strongly normalizing.
\end{theorem}
...
\begin{theorem}[Value Stability]
  For any value $v$ with dynamic typing $\Gamma ; \Delta \vdash v : A$ and $\Gamma \vdash A : s$, then $\Delta \triangleright s$.
\end{theorem}

\begin{theorem}[Dynamic Subject Reduction]
  For any dynamic typing $\Gamma ; \Delta \vdash m : A$ and dynamic reduction $m \Leadsto n$, then $\Gamma ; \Delta \vdash n : A$ is derivable.
\end{theorem}

\begin{theorem}[Dynamic Progress]
  For any dynamic typing $\epsilon ; \epsilon \vdash m : A$, then $m$ is a value or there exists $n$ such that $m \Leadsto n$.
\end{theorem}
