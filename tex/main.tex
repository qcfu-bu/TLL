...
\newcommand{\pairR}[3]{({#1},{#2})_{#3}}
\newcommand{\pairI}[3]{({#1},\{{#2}\})_{#3}}
\newcommand{\letin}[4]{\text{let }{#1}\text{ as }{#2}:={#3}\text{ in }{#4}}
...
\begin{center}
  \begin{tabular}{r l r l}
    variable    & $x, y, z, p$    &     &               \\
    sorts       & $s, r, t$       & ::= & $\Un$ | $\Ln$ \\
    expressions & $m,n,A,B,C,H,P$ & ::= & $x$ | $s$ \\
                &                 & \;| & $\PiR{t}{x : A}{B}$ | $\PiI{t}{x : A}{B}$
                                          | $\lamR{t}{x : A}{m}$ | $\lamI{t}{x : A}{m}$ | $m\ n$ \\
                &                 & \;| & $\SigR{t}{x : A}{B}$ | $\SigI{t}{x : A}{B}$
                                          | $\pairR{m}{n}{t}$ | $\pairI{m}{n}{t}$ \\
                &                 & \;| & $\letin{(x, y)}{A}{m}{n}$ \\
                &                 & \;| & $\with{A}{B}{t}$ | $\apair{m}{n}{t}$ | $\projL{m}$ | $\projR{m}$ \\
                &                 & \;| & $\iden{A}{m}{n}$ | $\refl{m}$ | $\J{A}{H}{P}$ | $\square$
  \end{tabular}
\end{center}
...
\begin{mathpar}
  ...
  { \Gamma \vdash \letin{(x, y)}{C}{m}{n} : C[m/z] }
  ...
  { \Gamma \vdash \J{B}{H}{P} : B[m/x,n/y,P/p] }
...
\begin{mathpar}
  ...
  { \Gamma ; \Delta_1 \dotcup \Delta_2 \vdash \letin{(x, y)}{C}{m}{n} : C[m/z] }
...
\begin{mathpar}
  ...
  { \Gamma ; \Delta_1 \dotcup \Delta_2 \vdash
    \letin{(x, y)}{C}{m}{n} \sim \letin{(x, y)}{\square}{m'}{n'} : C[m/z] }
