...
| dyn_rand Γ Δ :
  Δ ▷ U ->
  dyn_wf Γ Δ ->
  Γ ; Δ ⊢ Rand : Pi1 Unit (IO Nat) U
| dyn_return Γ Δ m A :
  Γ ; Δ ⊢ m : A ->
  Γ ; Δ ⊢ Return m : IO A
| dyn_letin Γ Δ1 Δ2 Δ m n A B s t :
  Δ1 ∘ Δ2 => Δ ->
  Γ ⊢ B : Sort t ->
  Γ ; Δ1 ⊢ m : IO A ->
  (A :: Γ) ; (A :{s} Δ2) ⊢ n : IO B.[ren (+1)] ->
  Γ ; Δ ⊢ LetIn m n : IO B
...
Proof with eauto using sta_wf. elim=>{Γ Δ}... Qed.
...
Proof with eauto using dyn_wf.
  ...
  { move=>Γ Δ1 Δ2 Δ A B m n s mrg tym ihm tyn ihn.
    apply: dyn_wf_merge... }
  { move=>Γ Δ1 Δ2 Δ m n A B s t mrg tyB tym ihm tyn ihn.
    inv ihn.
    apply: dyn_wf_merge... }
Qed.
