...
Lemma program_logical_step Γ Δ m n A :
  Γ ; Δ ⊢ m : A -> m ~>> n -> m ~>* n.
Proof with eauto using logical_step.
  ...
  { move=>Γ Δ1 Δ2 Δ A B m n t tyS ihS tym ihm tyn ihn n0 st. inv st.
    { apply: logical_red_pair1... }
    { apply: logical_red_pair1... } }
  { move=>Γ Δ1 Δ2 Δ A B C m n s r t tyC ihC tym ihm tyn ihn n0 st. inv st.
    { apply: logical_red_letin... }
    { apply: star1... }
    { apply: star1... } }
  { move=>Γ Δ1 Δ2 Δ A B C m n s r1 r2 t tyC ihC tym ihm tyn ihn n0 st. inv st.
    { apply: logical_red_letin... }
    { apply: star1... }
    { apply: star1... } }
  { move=>Γ Δ A B m t tym ihm n0 st. inv st.
    { apply: logical_red_fst... }
    { apply: star1... } }
  { move=>Γ Δ A B m t tym ihm n0 st. inv st.
    { apply: logical_red_snd... }
    { apply: star1... } }
  { move=>Γ Δ A B H P m n s h tyB tyH ihH tyP n0 st. inv st.
    have[P0[rdP vlP]]:=logical_vn tyP h.
    have tyP0:=logical_rd tyP rdP.
    have[n1 e]:=logical_id_canonical tyP0 (convR _ _)
                  (logical_red_closed rdP h) vlP. subst.
    apply: star_trans.
    apply: logical_red_rw...
    apply: star1... }
  { move=>Γ Δ A B m s eq tym ihm tyB n st.
    apply: ihm... }
Qed.
...
Theorem program_val_stability Γ Δ m A s :
...
Proof with eauto using key_impure.
  ...
  { move=>Γ Δ A B m t tym ihm tyA vl. inv vl. }
  { move=>Γ Δ A B H P m n s h tyB tyH ihH tyP tyB0 vl. inv vl. }
...
Theorem program_sr Γ Δ m n A :
  Γ ; Δ ⊢ m : A -> m ~>> n -> Γ ; Δ ⊢ n : A.
Proof with eauto using program_type, program_step, program_wf.
  move=>ty. elim: ty n=>{Γ Δ m A}...
  ...
  { move=>Γ Δ A B m n t tyS tym ihm tyn n0 st. inv st.
    ...
    apply: logical_conv_beta.
    apply: star_conv.
    apply: (program_logical_step tym)...
    apply: tyn.
    apply: logical_esubst...
    by autosubst. }
  ...
  { move=>Γ Δ A B H P m n s h tyB tyH ihH tyP n0 st. inv st.
    have[P0[rdP vlP]]:=logical_vn tyP h.
    have tyP0:=logical_rd tyP rdP.
    have[m0 e]:=logical_id_canonical tyP0 (convR _ _)
                  (logical_red_closed rdP h) vlP. subst.
...
Corollary program_rd Γ Δ m n A :
  Γ ; Δ ⊢ m : A -> m ~>>* n -> Γ ; Δ ⊢ n : A.
Proof with eauto.
