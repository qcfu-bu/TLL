(* The file presents various inversion lemmas for erasure. *)
...
Lemma erasure_var_form Γ Δ m x B :
...
Lemma erasure_lam0_form Γ Δ m n X B s :
...
Lemma erasure_lam1_form Γ Δ m n X B s :
...
Lemma erasure_pair0_form Γ Δ m m1' m2' A s :
...
Lemma erasure_pair1_form Γ Δ m m1' m2' A s :
...
Lemma erasure_apair_form Γ Δ m m1' m2' A s :
...
Lemma erasure_box_form Γ Δ m A : ~Γ ; Δ ⊢ m ~ Box : A.
Proof. move e:(Box)=>m' ty. elim: ty e=>//{Γ Δ m m' A}. Qed.

Lemma erasure_lam0_invX Γ Δ A1 A2 A3 B C m1 m2 s1 s2 t :
...
Lemma erasure_lam1_invX Γ Δ A1 A2 A3 B C m1 m2 s1 s2 t :
...
Proof with eauto.
  ...
  { move=>Γ Δ A B m m' s t k tym ihm A1 A2 A3 B0 m1 m2
    ...
    have tyA0t:=logical_rd H4 rd1.
    have tyA0s:=logical_rd H3 rd2.
    have e:=logical_sort_uniq tyA0t tyA0s. subst.
    exists s.
    apply: erasure_ctx_conv1.
    apply: conv_sym...
    exact: H3.
    apply: erasure_conv...
...
Lemma erasure_lam0_inv Γ Δ m m' A1 A2 A3 B s1 s2 :
  Γ ; Δ ⊢ Lam0 A2 m s2 ~ Lam0 A3 m' s2 : Pi0 A1 B s1 ->
  (A1 :: Γ) ; _: Δ ⊢ m ~ m' : B.
Proof with eauto.
  move=>ty.
  have[t/logical_pi0_inv[r[tyB _]]]:=program_valid (erasure_program_reflect ty).
  apply: erasure_lam0_invX...
Qed.

Lemma erasure_lam1_inv Γ Δ m m' A1 A2 A3 B s1 s2 :
  Γ ; Δ ⊢ Lam1 A2 m s2 ~ Lam1 A3 m' s2 : Pi1 A1 B s1 ->
  exists r, (A1 :: Γ) ; A1 :{r} Δ ⊢ m ~ m' : B.
Proof with eauto.
  move=>ty.
  have[t/logical_pi1_inv[r[tyB _]]]:=program_valid (erasure_program_reflect ty).
  apply: erasure_lam1_invX...
Qed.

Lemma erasure_pair0_invX Γ Δ A B m m' n n' s r t C :
...
Proof with eauto.
  ...
  { move=>Γ Δ A B m m' n t ty1 tym ihm tyn A0 B0 m0 m0' n0 n'
    ...
    have tym0:Γ ; Δ ⊢ m ~ m' : A0 by apply: erasure_conv; eauto.
    repeat split...
    apply: logical_conv.
...
Lemma erasure_pair1_invX Γ Δ A B m m' n n' s r t C :
...
Proof with eauto.
  ...
  { move=>Γ Δ1 Δ2 Δ A B m m' n n' t mrg ty1 tym ihm tyn ihn A0 B0
    ...
    have tym0:Γ ; Δ1 ⊢ m ~ m' : A0 by apply: erasure_conv; eauto.
    repeat split...
    apply: erasure_conv.
    apply: logical_conv_subst.
    all: eauto.
...
Lemma erasure_pair0_inv Γ Δ A B m m' n n' s r :
  Γ ; Δ ⊢ Pair0 m n s ~ Pair0 m' n' s : Sig0 A B r ->
  s = r /\ n' = Box /\ Γ ; Δ ⊢ m ~ m' : A /\ Γ ⊢ n : B.[m/].
Proof with eauto.
  move=>ty.
  have[t tyS]:=program_valid (erasure_program_reflect ty).
  apply: erasure_pair0_invX...
Qed.

Lemma erasure_pair1_inv Γ Δ A B m m' n n' s r :
...
Proof with eauto.
  move=>ty.
  have[t tyS]:=program_valid (erasure_program_reflect ty).
  apply: erasure_pair1_invX...
Qed.

Lemma erasure_apair_invX Γ Δ A B m m' n n' s r t C :
...
Lemma erasure_apair_inv Γ Δ A B m m' n n' s r :
  Γ ; Δ ⊢ APair m n s ~ APair m' n' s : With A B r ->
  s = r /\ Γ ; Δ ⊢ m ~ m' : A /\ Γ ; Δ ⊢ n ~ n' : B.
Proof with eauto.
  move=>ty.
  have[t tyS]:=program_valid (erasure_program_reflect ty).
  apply: erasure_apair_invX...
Qed.
