(* This file presents the heap progress theorem. *)

From mathcomp Require Import ssreflect ssrbool eqtype ssrnat seq.
From Coq Require Import ssrfun Classical Utf8.
Require Export AutosubstSsr ARS heap_sr.

Set Implicit Arguments.
Unset Strict Implicit.
Unset Printing Implicit Defensive.

(* A heap reduction conducted on sub-heap H1 of H can be reframed as a
   heap reduction conducted on the entire heap H. *)
Lemma heap_step_merge H1 H1' H2 H m n :
  H1 ; m ~>> H1' ; n -> H1 ∘ H2 => H -> exists H' n', H ; m ~>> H' ; n'.
Proof with eauto using heap_step, pad, merge.
  ...
  { move=>H H' m n n' st ihn H2 H0 mrg.
    ...
    exists H0'. exists (App (Ptr m) nx)... }
  { move=>H H' l m n s fr H2 H0 mrg.
    have[H4[fr' mrg']]:=lookup_merge fr mrg.
...
Lemma lookup_pair0_canonical H H' m n l t :
  wr_heap H -> lookup H l (Pair0 m n t) H' ->
...
Lemma lookup_pair1_canonical H H' m n l t :
  wr_heap H -> lookup H l (Pair1 m n t) H' ->
...
(* Theorem 15 (Heap Progress) *)
Theorem heap_prog Γ H x y z A :
  Γ ; H ⊢ x ~ y ~ z : A -> wr_heap H ->
  (exists H' z', H ; z ~>> H' ; z') \/ (exists l, z = Ptr l).
Proof with eauto using heap_step.
  move=>{Γ H x y z A}[Γ Δ H x y z A]dN er.
  elim: er H z dN=>{Γ Δ x y A}.
  { move=>Γ Δ x s A wf shs dhs H z dN.
    exfalso. apply: program_has_empty... }
  { move=>Γ Δ A B m m' s _ erm ihm H z dN rs wr.
    inv rs.
    { left. exists (Lam0 Box m0 s :{s} H). exists (Ptr (size H))... }
    { right. exists l... } }
  { move=>Γ Δ A B m m' s t _ erm ihm H z dN rs wr.
    inv rs.
    { left. exists (Lam1 Box m0 s :{s} H). exists (Ptr (size H))... }
    { right. exists l... } }
  { move=>Γ Δ A B m m' n s erm ihm tyn H z dN rs wr. inv rs.
    { have[[H'[z' st]]|[l e]]:=ihm _ _ dN H4 wr.
      { left. exists H'. exists (App z' Box)... }
      { subst. have vl':=wr_resolve_ptr wr H4.
        have vl:=erasure_program_val erm vl'.
        have[A0[m0 e]]:=program_pi0_canonical (erasure_program_reflect erm) (convR _ _) dN vl. subst.
    ...
    { inv H2.
      { have vl:=wr_lookup_program_val H1 wr. inv vl. }
      { have wr':=lookup_wr H1 wr.
        have[wr1 wr2]:=wr_merge_inv H7 wr'.
        have//:=resolve_wr_box wr2 H11. }
      { exfalso. apply: lookup_wr_ptr... } } }
  { move=>Γ Δ1 Δ2 Δ A B m m' n n' s mrg erm ihm ern ihn H z dN rs wr.
    have[dN1 dN2]:=program_empty_split mrg dN. inv rs.
    { have//:=erasure_box_form ern. }
    { left. have[wr1 wr2]:=wr_merge_inv H5 wr.
      have[[H1'[m1 st1]]|[l1 e1]]:=ihm _ _ dN1 H8 wr1.
      { have[H'[m0' st']]:=heap_step_merge st1 H5.
        exists H'. exists (App m0' n0)... }
      { subst. have[[H2'[m2 st2]]|[l2 e2]]:=ihn _ _ dN2 H9 wr2.
        { have[H'[m2' st']]:=heap_step_merge st2 (merge_sym H5).
          exists H'. exists (App (Ptr l1) m2')... }
        { subst.
          have vm':=wr_resolve_ptr wr1 H8.
          have vm:=erasure_program_val erm vm'.
          have[A0[m0 e]]:=program_pi1_canonical (erasure_program_reflect erm) (convR _ _) dN1 vm. subst.
          have[m1 e]:=erasure_lam1_canonical erm. subst. inv H8. inv H7.
          { have[H6[fr' mrg']]:=lookup_merge H4 H5.
            exists H6. exists m2.[Ptr l2/]... }
          { exfalso. apply: lookup_wr_ptr... } } } }
    { right. exists l... } }
  { move=>Γ Δ A B m m' n t tyS erm ihm tyn H z dN rs wr. inv rs.
    { have[[H'[m1 st]]|[l e]]:=ihm _ _ dN H4 wr.
      { left. exists H'. exists (Pair0 m1 Box t)... }
      { subst. left.
        exists (Pair0 (Ptr l) Box t :{t} H).
        exists (Ptr (size H))... } }
    { right. exists l... } }
  { move=>Γ Δ1 Δ2 Δ A B m m' n n' t mrg tyS erm ihm ern ihn H z dN rs wr.
    have[dN1 dN2]:=program_empty_split mrg dN. inv rs.
    { left. have[wr1 wr2]:=wr_merge_inv H8 wr.
      have[[H1'[m1 st1]]|[l1 e1]]:=ihm _ _ dN1 H9 wr1.
      { have[H'[m0' st']]:=heap_step_merge st1 H8.
        exists H'. exists (Pair1 m0' n0 t)... }
      { subst. have[[H2'[m2 st2]]|[l2 e2]]:=ihn _ _ dN2 H10 wr2.
        { have[H'[n0' st']]:=heap_step_merge st2 (merge_sym H8).
          exists H'. exists (Pair1 (Ptr l1) n0' t)... }
        { subst.
          exists (Pair1 (Ptr l1) (Ptr l2) t :{t} H).
          exists (Ptr (size H))... } } }
    { right. exists l... } }
  { move=>Γ Δ1 Δ2 Δ A B C m m' n n' s r t mrg tyC erm ihm ern _ H z dN rs wr.
    have[dN1 dN2]:=program_empty_split mrg dN. inv rs.
    { left. have[wr1 wr2]:=wr_merge_inv H5 wr.
      have[[H1'[m1 st]]|[l e]]:=ihm _ _ dN1 H8 wr1.
      { have[H'[m0' st']]:=heap_step_merge st H5.
        exists H'. exists (LetIn Box m0' n0)... }
      { subst.
        have vm':=wr_resolve_ptr wr1 H8.
        have vm:=erasure_program_val erm vm'.
        have[m1[m2 e]]:=program_sig0_canonical (erasure_program_reflect erm) (convR _ _) dN1 vm. subst.
        have[m0 e]:=erasure_pair0_canonical erm. subst. inv H8. inv H7.
        { have[H6[fr' mrg']]:=lookup_merge H4 H5.
          have[lm[e _]]:=lookup_pair0_canonical wr fr'. subst.
          exists H6. exists (n0.[Box,Ptr lm/])... }
        { exfalso. apply: lookup_wr_ptr... } } }
    { right. exists l... } }
  { move=>Γ Δ1 Δ2 Δ A B C m m' n n' s r1 r2 t mrg tyC erm ihm ern _ H z dN rs wr.
    have[dN1 dN2]:=program_empty_split mrg dN. inv rs.
    { left. have[wr1 wr2]:=wr_merge_inv H5 wr.
      have[[H1'[m1 st]]|[l e]]:=ihm _ _ dN1 H8 wr1.
      { have[H'[m0' st']]:=heap_step_merge st H5.
      ...
      { subst.
        have vm':=wr_resolve_ptr wr1 H8.
        have vm:=erasure_program_val erm vm'.
        have[m1[m2 e]]:=program_sig1_canonical (erasure_program_reflect erm) (convR _ _) dN1 vm. subst.
        have[m1'[m2' e]]:=erasure_pair1_canonical erm. subst. inv H8. inv H7.
        { have[H6[fr' mrg']]:=lookup_merge H4 H5.
          have[lm[ln[e1 e2]]]:=lookup_pair1_canonical wr fr'. subst.
          exists H6. exists (n0.[Ptr ln,Ptr lm/])... }
        { exfalso. apply: lookup_wr_ptr... } } }
    { right. exists l... } }
  { move=>Γ Δ A B m m' n n' t _ erm ihm ern ihn H z dN rs wr. inv rs.
    { left. exists (APair m0 n0 t :{t} H). exists (Ptr (size H))... }
    { right. exists l... } }
  { move=>Γ Δ A B m m' t erm ihm H z dN rs wr. inv rs.
    { left. have[[H'[m1 st]]|[l e]]:=ihm _ _ dN H4 wr.
      { exists H'. exists (Fst m1)... }
      { subst.
        have vm':=wr_resolve_ptr wr H4.
        have vm:=erasure_program_val erm vm'.
        have[m1[m2 e]]:=program_with_canonical (erasure_program_reflect erm) (convR _ _) dN vm. subst.
        have[m1'[m2' e]]:=erasure_apair_canonical erm. subst. inv H4. inv H5.
        { exists H'. exists m0... }
        { exfalso. apply: lookup_wr_ptr... } } }
    { right. exists l... } }
  { move=>Γ Δ A B m m' t erm ihm H z dN rs wr. inv rs.
    { left. have[[H'[m1 st]]|[l e]]:=ihm _ _ dN H4 wr.
      ...
      { subst.
        ...
        { exfalso. apply: lookup_wr_ptr... } } }
    { right. exists l... } }
  { move=>Γ Δ A B x x' P m n s h tyB erx ihx tyP H z dN rs wr. inv rs.
    { left. exists H. exists m0... }
    { right. exists l... } }
  { move=>Γ Δ A B m m' s eq erm ihm tyB H z dN rs wr... }
Qed.
