...
Proof with eauto.
  ...
  { move=>Γ s wf e A B s0 eq.
    exfalso. solve_conv. }
  { move=>Γ x A wf shs e A0 B s eq vl. subst. inv shs. }
  { move=>Γ A B s r t tyA ihA tyB ihB e A0 B0 s0 eq.
    exfalso. solve_conv. }
  { move=>Γ A B s r t tyA ihA tyB ihB e A0 B0 s0 eq.
    exfalso. solve_conv. }
  { move=>Γ A B m s tym ihm e1 A0 B0 s0/pi0_inj[eqA[eqB e2]] vl. subst.
    exists A. exists m... }
  { move=>Γ A B m s tym ihm e1 A0 B0 s0 eq.
    exfalso. solve_conv. }
  { move=>Γ A B m n s tym ihm tyn ihn e A0 B0 s0 eq vl. inv vl. }
  { move=>Γ A B m n s tym ihm tyn ihn e A0 B0 s0 eq vl. inv vl. }
  { move=>Γ A m n s tyA ihA tym ihm tyn ihn e A0 B s0 eq.
    exfalso. solve_conv. }
  { move=>Γ A m tym ihm e A0 B s eq.
    exfalso. solve_conv. }
  { move=>Γ A B H P m n s tyB ihB tyH ihH tyP ihP e A0 B0 s0 eq vl. inv vl. }
...
Proof with eauto.
  ...
  { move=>Γ s wf e A x y eq.
    exfalso. solve_conv. }
  { move=>Γ x A wf shs e. subst. inv shs. }
  { move=>Γ A B s r t tyA ihA tyB ihB e A0 x y eq.
    exfalso. solve_conv. }
  { move=>Γ A B s r t tyA ihA tyB ihB e A0 x y eq.
    exfalso. solve_conv. }
  { move=>Γ A B m s tym ihm e A0 x y eq.
    exfalso. solve_conv. }
  { move=>Γ A B m s tym ihm e A0 x y eq.
    exfalso. solve_conv. }
  { move=>Γ A B m n s tym ihm tyn ihn e A0 x y eq vl. inv vl. }
  { move=>Γ A B m n s tym ihm tyn ihn e A0 x y eq vl. inv vl. }
  { move=>Γ A m n s tyA ihA tym ihm tyn ihn e A0 x y eq.
    exfalso. solve_conv. }
  { move=>Γ A m tym ihm e A0 x y eq vl.
    exists m... }
  { move=>Γ A B H P m n s tyB ihB tyH ihH tyP ihP e A0 x y eq vl. inv vl. }
